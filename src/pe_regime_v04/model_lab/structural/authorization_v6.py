"""Isolated V6 authorization loader; all earlier authority bytes stay frozen."""
...
AUTHORITY_POLICY_V6_NAME = "authority_policy_v6.py"


def load_structural_execution_authorization_v6(
    project_root: Path,
    *,
    external_policy_sha256: str,
    scope: core.AuthorizationScope = "SYNTHETIC_NO_SCORE",
) -> core.StructuralExecutionAuthorization:
    """Mint a V6 capability while retaining the audited V4 base locks."""
    ...
    policy_path = (
        root / f"src/pe_regime_v04/model_lab/structural/{AUTHORITY_POLICY_V6_NAME}"
    ).resolve()
    policy = core._load_authority_policy(policy_path, expected_raw_sha256=external_policy_sha256)
    ...
    for path in (decision_path, base_path, snapshot_path, policy_path, activation_path):
        try:
            ...
            raise core.StructuralContractError(
                "V6 authorization path escapes project root"
            ) from exc
    ...
    activation = core._read_json(activation_path, context="V6 audit activation")
    ...
    if scope == "FORMAL_SPENT" and not spent:
        raise core.StructuralContractError(
            "V6 formal spent execution is blocked pending independent audit GO"
        )
    values: dict[str, Any] = {
        "scope": scope,
        "project_root": root,
