"""Externally pinned, stdlib-only launcher for the score-free V7 preflight.
...
SOURCE_LOCK_TEXT = PROJECT_ROOT_TEXT + (
    "\\scripts\\model_lab\\causal_valuation_tcn_v7\\V7_SOURCE_LOCK.json"
)
RUNTIME_LOCK_TEXT = PROJECT_ROOT_TEXT + (
    "\\scripts\\model_lab\\causal_valuation_tcn_v7\\V7_RUNTIME_LOCK.json"
)
LAUNCHER_TEXT = PROJECT_ROOT_TEXT + (
    "\\scripts\\model_lab\\causal_valuation_tcn_v7\\trusted_launcher.py"
)
STAGE_TEXT = PROJECT_ROOT_TEXT + "\\outputs\\.causal_valuation_tcn_v7_verified_stage"
...
PINNED_SOURCE_LOCK_SHA256 = "243e3ae380a2c3077bbfde1469a9e2b9ca262f0387c2dda508e7b42cd5c8ff12"
PINNED_RUNTIME_LOCK_SHA256 = "dee5af55de401d8853268db28a2e9b4a3616bd551afc15ed97768a060380e360"

SOURCE_SCHEMA = "expected_pe.causal_valuation_tcn_v7.source_lock.v1"
RUNTIME_SCHEMA = "expected_pe.causal_valuation_tcn_v7.runtime_lock.v1"
PREIMPORT_SCHEMA = "expected_pe.causal_valuation_tcn_v7.preimport_attestation.v2"
SOURCE_PACKAGE_PREFIX = "research.model_zoo.causal_valuation_tcn_v7"
...
def _verify_source_lock() -> tuple[dict[str, Any], dict[str, bytes], dict[str, Any]]:
    ...
    lock = _read_pinned_lock(
        SOURCE_LOCK_TEXT, PINNED_SOURCE_LOCK_SHA256, label="V7 source lock"
    )
    required = {
        "schema_version",
        "status",
...
def _distribution_receipt(
    ...
    seen: set[str] = set()
    duplicate_paths = 0
    members = hashlib.sha256(b"expected_pe.causal_valuation_tcn_v7.record_members.v1\0")
    ...
    *,
    ...
    for name in sorted(expected_versions):
        ...
        for key, value in members.items():
            ...
            file_map[key] = value
    closure = hashlib.sha256(b"expected_pe.causal_valuation_tcn_v7.record_closure.v1\0")
...
def _verify_runtime_lock() -> tuple[dict[str, Any], dict[str, dict[str, Any]], dict[str, Any]]:
    lock = _read_pinned_lock(
        RUNTIME_LOCK_TEXT, PINNED_RUNTIME_LOCK_SHA256, label="V7 runtime lock"
    )
    required = {
...
class VerifiedBytesFinder(importlib.abc.MetaPathFinder):
    """Only loader authorized to create governed V7 Python modules."""

    def __init__(self, source_bytes: Mapping[str, bytes]) -> None:
        self.modules: dict[str, tuple[bytes, str, bool]] = {}
        prefix = "research/model_zoo/causal_valuation_tcn_v7/"
...
def _write_stage(captured: Mapping[str, bytes]) -> None:
    ...
    if stage.parent != outputs or stage.name != ".causal_valuation_tcn_v7_verified_stage":
...
def _controlled_environment(
    *, receipt: Mapping[str, Any], finder: VerifiedBytesFinder
    ...
    environment.update({
        ...
        "CVTCN_V7_LIVE_PROJECT_ROOT": PROJECT_ROOT_TEXT,
        "CVTCN_V7_SOURCE_STAGE": STAGE_TEXT,
        "CVTCN_V7_PREIMPORT_ATTESTATION_SHA256": _sha256_bytes(receipt_text.encode("ascii")),
        "CVTCN_V7_PREIMPORT_RECEIPT_JSON": receipt_text,
        "CVTCN_V7_VERIFIED_MODULE_ORIGINS_JSON": _canonical(finder.origin_records()).decode("ascii"),
    })
    if "CVTCN_V7_TEST_OUTPUT_CONTAINER" in os.environ:
        environment["CVTCN_V7_TEST_OUTPUT_CONTAINER"] = os.environ["CVTCN_V7_TEST_OUTPUT_CONTAINER"]
    return environment
...
def _worker(task: str, device: str | None) -> int:
    ...
    if os.environ.get("CVTCN_V7_PREIMPORT_ATTESTATION_SHA256") != receipt_sha:
        raise PreimportError("parent/worker pre-import attestation differs")
    if os.environ.get("CVTCN_V7_PREIMPORT_RECEIPT_JSON") != receipt_text:
    ...
    if os.environ.get("CVTCN_V7_VERIFIED_MODULE_ORIGINS_JSON") != expected_origins:
    ...
    if task == "tests":
        ...
        relative = "tests/model_lab/causal_valuation_tcn_v7/torch_checks.py"
        namespace = {
            "__name__": "__main__",
...
def _parent(*, preflight: bool) -> int:
    ...
    try:
        ...
        if not preflight:
            output = {
                "schema_version": "expected_pe.causal_valuation_tcn_v7.trusted_tests.v1",
        ...
        output = {
            "schema_version": "expected_pe.causal_valuation_tcn_v7.trusted_preflight.v1",
