"""Hard-coded stdlib-only R4 pre-import verified-bytes bootstrap."""
...
SELF_TEXT = PROJECT_TEXT + (
    r"\scripts\model_lab\observable_state_bce_dgp_tournament_v2_r4"
    r"\trusted_bootstrap.py"
)
PACKAGE_TEXT = PROJECT_TEXT + (
    r"\research\model_zoo\observable_state_bce_dgp_tournament_v2_r4"
)
...
PRECOMMIT_TEXT = PROJECT_TEXT + (
    ...
    r"score_free_precommit_r4_20260821"
)
...
REPARSE = getattr(stat, "FILE_ATTRIBUTE_REPARSE_POINT", 0x400)
stage = Path(os.environ["R4_STAGE_ROOT"])
manifest_path = stage / "EXECUTABLE_MANIFEST.json"
manifest_raw = manifest_path.read_bytes()
if hashlib.sha256(manifest_raw).hexdigest() != os.environ["R4_EXECUTABLE_MANIFEST_RAW_SHA256"]:
...
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r4 import bootstrap_entry
...
def _load_manifest() -> tuple[dict[str, object], bytes]:
    ...
    expected_stage_sources = {
        f"research/model_zoo/observable_state_bce_dgp_tournament_v2_r4/{name}"
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="r4_source_only_") as temporary:
        ...
        environment = {
            ...
            "R4_BOOTSTRAP_COMMAND": args.command,
            "R4_BOOTSTRAP_PARENT_NONCE": secrets.token_hex(32),
            "R4_EXECUTABLE_MANIFEST_RAW_SHA256": hashlib.sha256(manifest_raw).hexdigest(),
        ...
        if args.command == "reserve_authenticated":
            environment["R4_RESERVATION_ACTIVATION"] = RESERVATION_ACTIVATION
