"""Run exact score-free probes and atomically freeze the R4 precommit."""
...
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r4.precommit import (
...
TEST_MAIN = "tests/model_lab/test_observable_state_bce_dgp_tournament_v2_r4.py"
TEST_TRANSACTION = (
    "tests/model_lab/test_observable_state_bce_dgp_tournament_v2_r4_transaction.py"
)
PROBE_NODEIDS = {
    ...
    "R3_NO_GO_full_bundle_supersession_exact": [
        f"{TEST_MAIN}::test_R3_NO_GO_full_bundle_supersession_exact"
    ],
...
def main() -> int:
    if str(Path(sys.argv[0])) != str(SELF) or str(Path(__file__)) != str(SELF):
        raise RuntimeError("R4 freeze must be invoked by its exact absolute path")
    if (
        ...
        raise RuntimeError("R4 freeze interpreter/tool/probe universe differs")
    ...
    ruff_targets = [
        "research/model_zoo/observable_state_bce_dgp_tournament_v2_r4",
        "scripts/model_lab/observable_state_bce_dgp_tournament_v2_r4",
        TEST_MAIN,
        TEST_TRANSACTION,
    ]
    ...
    if sha256_file(registry) != registry_before:
        raise RuntimeError("registry changed during score-free R4 freeze")
