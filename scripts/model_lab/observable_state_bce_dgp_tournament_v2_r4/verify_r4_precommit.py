"""Invoke the one exact standalone R4 bootstrap and print its sealed receipt."""
...
def main() -> int:
    ...
    if sys.argv[0] != SELF or str(Path(__file__)) != SELF:
        raise RuntimeError("R4 verifier must be called by its exact absolute path")
    parent = os.environ
    environment = {
    ...
    if (
        ...
        raise RuntimeError("R4 bootstrap receipt differs")
