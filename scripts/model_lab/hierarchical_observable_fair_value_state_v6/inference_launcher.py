"""Stdlib-first exact runtime launcher preflight for future H-OFS V6 inference.
...
def _current_affinity_mask() -> int:
    if os.name != "nt":
        raise RuntimeError("H-OFS V6 launcher requires Windows")
...
def _preimport_guard() -> dict[str, object]:
    ...
    if observed != expected:
        ...
        raise RuntimeError(f"H-OFS V6 pre-import runtime drift: {drifted}")
    return observed
...
def main() -> int:
    ...
    from research.model_zoo.hierarchical_observable_fair_value_state_v6.runtime import (
    ...
    print(
        json.dumps(
            {
                "status": "PASS_EXACT_HOFS_V6_INFERENCE_LAUNCHER_PREFLIGHT_ONLY",
                "preimport": preimport,
