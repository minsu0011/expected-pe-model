"""Build and atomically freeze the score-blind H-OFS V6 DGP-R4 preflight."""
...
DEFAULT_OUTPUT_ROOT = (
    PROJECT_ROOT
    / "outputs/model_zoo_hierarchical_observable_fair_value_state_v6_dgp_r4_design_preflight_20260821"
)
FINAL_FILE_UNIVERSE = (
    "ACCESS_RECEIPT.json",
    "AUDIT_CLOSURE.json",
    ...
    def reject_duplicates(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, value in pairs:
            if type(key) is not str or key in output:
                raise RuntimeError(f"H-OFS V6 duplicate/invalid JSON key: {name}:{key}")
            output[key] = value
        return output
    ...
    try:
        payload = json.loads(
            ...
            parse_constant=lambda value: (_ for _ in ()).throw(
                RuntimeError(f"H-OFS V6 non-finite JSON value: {name}:{value}")
            ),
        )
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise RuntimeError(f"H-OFS V6 invalid JSON: {name}") from error
    if type(payload) is not dict:
        raise RuntimeError(f"H-OFS V6 JSON root is not an object: {name}")
    return payload
...
def _require_sha256(value: object, *, label: str) -> str:
    if (
        ...
        raise RuntimeError(f"H-OFS V6 {label} is not lowercase SHA-256")
    return value
...
def build_preflight_payload(project_root: Path = PROJECT_ROOT) -> dict[str, Any]:
    ...
    source_audit = run_source_audit_v6(project_root)
    if not source_audit.passed:
        raise RuntimeError("H-OFS V6 source isolation failed")
    input_closure = build_r4_input_closure_v6(project_root)
    resource_receipt = capture_runtime_receipt_v6(purpose="PREFLIGHT")
    return {
        "schema_version": "expected_pe.hofs_v6.dgp_r4_score_blind_preflight.v1",
        ...
        "v5_independent_no_go_audit_binding": dict(V5_INDEPENDENT_AUDIT_BINDING),
        "v5_design_freeze_binding": dict(V5_DESIGN_FREEZE_BINDING),
...
def build_design_bundle_bytes(project_root: Path = PROJECT_ROOT) -> dict[str, bytes]:
    ...
    design_lock = sealed_payload(
        {
            "schema_version": "expected_pe.hofs_v6.dgp_r4_design_lock.v1",
            "status": "PASS_V6_DGP_R4_BATCH_MAINTENANCE_DESIGN_LOCK_SCORE_BLIND",
            "design_contract_sha256": contract_sha256(),
            "design_contract": contract_payload(),
            "v5_independent_no_go_audit_binding": dict(V5_INDEPENDENT_AUDIT_BINDING),
            "v5_design_freeze_binding": dict(V5_DESIGN_FREEZE_BINDING),
    ...
    source_closure = sealed_payload(
        {
            "schema_version": "expected_pe.hofs_v6.source_closure.v1",
            "status": "PASS_COMPLETE_V6_V5_NO_GO_LINEAGE_AND_PARENT_RUNTIME_SOURCE_CLOSURE",
    ...
    audit_closure = sealed_payload(
        {
            "schema_version": "expected_pe.hofs_v6.v5_no_go_audit_closure.v1",
            "status": "PASS_EXACT_V5_NO_GO_P1_BOUND_AS_V6_REPAIR_INPUT",
            "v5_independent_no_go_audit_binding": dict(V5_INDEPENDENT_AUDIT_BINDING),
            "v5_design_freeze_binding": dict(V5_DESIGN_FREEZE_BINDING),
            "v5_severity_counts": {"P0": 0, "P1": 1, "P2": 0},
            "v5_finding_count": 1,
            "v5_failed_check": "noncanonical_requested_block_order_rejected",
            "v6_repair_scope": "P1_ORDER_CUSTODY_ONLY",
        }
    )
    input_closure = sealed_payload(input_closure_payload)
    resource_receipt = sealed_payload(
        {
            "schema_version": "expected_pe.hofs_v6.resource_receipt.v1",
    ...
    quality_receipt = sealed_payload(
        {
            "schema_version": "expected_pe.hofs_v6.quality_receipt.v1",
            "status": "PASS_V6_ORDER_CUSTODY_BLOCK_WARMUP_AND_PUBLIC_SCHEMA_PREFLIGHT",
            "pytest": {
                "bytecode_disabled": True,
                "cache_provider_disabled": True,
                "focused_v6_passed": 39,
                "focused_v6_failed": 0,
                "v1_v2_v3_v4_v5_regression_passed": 105,
                "v1_v2_v3_v4_v5_regression_failed": 0,
            },
    ...
    access_receipt = sealed_payload(
        {
            "schema_version": "expected_pe.hofs_v6.access_receipt.v1",
    ...
    report = (
        "# H-OFS V6 DGP-R4 Score-Blind Preflight\n\n"
        "Status: `PASS_V6_DGP_R4_DESIGN_INPUT_PREFLIGHT_NO_MODEL_LAUNCH`\n\n"
        f"Contract: `{contract_sha256()}`\n\n"
        "V6 preserves the frozen V5 estimator, geometry, runtime, warmup policy, final-weight "
        "KKT certificate, exact types, and live state/mask custody. It repairs the independently "
        "demonstrated V5 P1 by rejecting any incoming requested block that is not already in "
        "exact decision-date/entity stable-mergesort order, without silently sorting. Separate "
    ...
    manifest = sealed_payload(
        {
            "schema_version": "expected_pe.hofs_v6.dgp_r4_design_preflight_manifest.v1",
            "status": "PASS_V6_DGP_R4_PREFLIGHT_READY_NO_MODEL_LAUNCH",
            "design_contract_sha256": contract_sha256(),
            "v5_independent_no_go_audit_binding": dict(V5_INDEPENDENT_AUDIT_BINDING),
            "v5_design_freeze_binding": dict(V5_DESIGN_FREEZE_BINDING),
            "r4_input_binding": dict(R4_INPUT_BINDING),
    ...
    seal = sealed_payload(
        {
            "schema_version": "expected_pe.hofs_v6.dgp_r4_design_preflight_seal.v1",
            "status": "PASS_V6_DGP_R4_DESIGN_INPUT_PREFLIGHT_SEALED_NO_MODEL_LAUNCH",
    ...
    if tuple(sorted(bundle)) != FINAL_FILE_UNIVERSE:
        raise RuntimeError("H-OFS V6 design bundle universe drifted")
    return bundle
...
def verify_design_bundle(
    ...
    *,
    ...
    if raw_directory.is_symlink() or _windows_reparse_point(raw_directory):
        raise RuntimeError("H-OFS V6 design bundle reparse root is forbidden")
    directory = raw_directory.resolve()
    if not directory.is_dir() or _windows_reparse_point(directory):
        raise RuntimeError("H-OFS V6 design bundle root is not a regular directory")
    children = tuple(directory.iterdir())
    if any(
        path.is_symlink() or _windows_reparse_point(path) or not path.is_file() for path in children
    ):
        raise RuntimeError("H-OFS V6 design bundle contains a non-regular child")
    actual_names = tuple(sorted(path.name for path in children))
    if len({name.casefold() for name in actual_names}) != len(actual_names):
        raise RuntimeError("H-OFS V6 design bundle names are not unique")
    if actual_names != FINAL_FILE_UNIVERSE:
        raise RuntimeError("H-OFS V6 frozen design universe drifted")
    ...
    if _raw_sha256(contents["CHECKSUMS.sha256"]) != expected_checksums_raw_sha256:
        raise RuntimeError("H-OFS V6 external checksum receipt drifted")
    ...
    try:
        ...
        raise RuntimeError("H-OFS V6 checksum ledger encoding drifted") from error
    if len(checksum_lines) != len(expected_ledger_names):
        raise RuntimeError("H-OFS V6 checksum entry count drifted")
    recorded_names: list[str] = []
    for position, line in enumerate(checksum_lines):
        parts = line.split("  ", maxsplit=1)
        if len(parts) != 2:
            raise RuntimeError("H-OFS V6 checksum syntax drifted")
        digest, name = parts
        _require_sha256(digest, label="checksum entry")
        if name != expected_ledger_names[position] or name in recorded_names:
            raise RuntimeError("H-OFS V6 checksum order/universe/uniqueness drifted")
        if "/" in name or "\\" in name or Path(name).name != name:
            raise RuntimeError("H-OFS V6 checksum path syntax drifted")
        if _raw_sha256(contents[name]) != digest:
            raise RuntimeError(f"H-OFS V6 frozen checksum drifted: {name}")
    ...
    if tuple(sorted(canonical_bundle)) != FINAL_FILE_UNIVERSE:
        raise RuntimeError("H-OFS V6 canonical bundle universe drifted")
    for name in FINAL_FILE_UNIVERSE:
        if contents[name] != canonical_bundle[name]:
            raise RuntimeError(f"H-OFS V6 canonical cross-binding drifted: {name}")

    manifest = parsed["MANIFEST.json"]
    seal = parsed["SEAL_RECEIPT.json"]
    if manifest.get("design_contract_sha256") != contract_sha256():
        raise RuntimeError("H-OFS V6 manifest contract binding drifted")
    if canonical_json_bytes(manifest.get("v5_independent_no_go_audit_binding")) != (
        canonical_json_bytes(V5_INDEPENDENT_AUDIT_BINDING)
    ):
        raise RuntimeError("H-OFS V6 manifest V5-audit binding drifted")
    if canonical_json_bytes(manifest.get("v5_design_freeze_binding")) != (
        canonical_json_bytes(V5_DESIGN_FREEZE_BINDING)
    ):
        raise RuntimeError("H-OFS V6 manifest V5-design binding drifted")
    if canonical_json_bytes(manifest.get("r4_input_binding")) != canonical_json_bytes(
        R4_INPUT_BINDING
    ):
        raise RuntimeError("H-OFS V6 manifest R4-input binding drifted")
    if manifest.get("expected_final_file_universe") != list(FINAL_FILE_UNIVERSE):
        raise RuntimeError("H-OFS V6 manifest universe binding drifted")
    if seal.get("design_contract_sha256") != contract_sha256():
        raise RuntimeError("H-OFS V6 seal contract binding drifted")
    if canonical_json_bytes(seal.get("v5_independent_no_go_audit_binding")) != (
        canonical_json_bytes(V5_INDEPENDENT_AUDIT_BINDING)
    ):
        raise RuntimeError("H-OFS V6 seal V5-audit binding drifted")
    if canonical_json_bytes(seal.get("v5_design_freeze_binding")) != canonical_json_bytes(
        V5_DESIGN_FREEZE_BINDING
    ):
        raise RuntimeError("H-OFS V6 seal V5-design binding drifted")
    if canonical_json_bytes(seal.get("r4_input_binding")) != canonical_json_bytes(R4_INPUT_BINDING):
        raise RuntimeError("H-OFS V6 seal R4-input binding drifted")
    if seal.get("expected_final_file_universe") != list(FINAL_FILE_UNIVERSE):
        raise RuntimeError("H-OFS V6 seal universe binding drifted")
    return {
        "status": "PASS_EXACT_V6_DESIGN_BUNDLE_CLOSURE",
...
def freeze_design_bundle(output_root: Path = DEFAULT_OUTPUT_ROOT) -> dict[str, Any]:
    ...
    if raw_target.is_symlink() or _windows_reparse_point(raw_target):
        raise RuntimeError("H-OFS V6 freeze target reparse point is forbidden")
    target = raw_target.resolve()
    outputs_root = (PROJECT_ROOT / "outputs").resolve()
    if target.parent != outputs_root or target != DEFAULT_OUTPUT_ROOT.resolve():
        raise RuntimeError("H-OFS V6 freeze target must be the exact declared outputs child")
    staging = outputs_root / f".{target.name}.staging"
    if target.exists() or staging.exists():
        raise FileExistsError("H-OFS V6 freeze target or staging root already exists")
