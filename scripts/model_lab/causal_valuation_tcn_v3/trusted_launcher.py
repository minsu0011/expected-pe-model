"""Externally pinned, stdlib-only launcher for the score-free V3 preflight.
...
SOURCE_LOCK_TEXT = PROJECT_ROOT_TEXT + (
    "\\scripts\\model_lab\\causal_valuation_tcn_v3\\V3_SOURCE_LOCK.json"
)
RUNTIME_LOCK_TEXT = PROJECT_ROOT_TEXT + (
    "\\scripts\\model_lab\\causal_valuation_tcn_v3\\V3_RUNTIME_LOCK.json"
)
LAUNCHER_TEXT = PROJECT_ROOT_TEXT + (
    "\\scripts\\model_lab\\causal_valuation_tcn_v3\\trusted_launcher.py"
)
STAGE_TEXT = PROJECT_ROOT_TEXT + "\\outputs\\.causal_valuation_tcn_v3_verified_stage"
...
PINNED_SOURCE_LOCK_SHA256 = "fc1d253976222ee8239fdad42bd40abe8d25521aa92aec87394760736ad48e5a"
PINNED_RUNTIME_LOCK_SHA256 = "577b5a29cfc9e01347de516ef64f805381caa3203e0b5de4f83b833a7079cee1"

SOURCE_SCHEMA = "expected_pe.causal_valuation_tcn_v3.source_lock.v1"
RUNTIME_SCHEMA = "expected_pe.causal_valuation_tcn_v3.runtime_lock.v1"
PREIMPORT_SCHEMA = "expected_pe.causal_valuation_tcn_v3.preimport_attestation.v2"
SOURCE_PACKAGE_PREFIX = "research.model_zoo.causal_valuation_tcn_v3"
...
def _verify_source_lock() -> tuple[dict[str, Any], dict[str, bytes], dict[str, Any]]:
    ...
    lock = _read_pinned_lock(
        SOURCE_LOCK_TEXT, PINNED_SOURCE_LOCK_SHA256, label="V3 source lock"
    )
    required = {
        "schema_version",
        "status",
...
def _distribution_receipt(
    ...
    seen: set[str] = set()
    duplicate_paths = 0
    members = hashlib.sha256(b"expected_pe.causal_valuation_tcn_v3.record_members.v1\0")
    ...
    *,
    ...
    for name in sorted(expected_versions):
        ...
        for key, value in members.items():
            ...
            file_map[key] = value
    closure = hashlib.sha256(b"expected_pe.causal_valuation_tcn_v3.record_closure.v1\0")
...
def _verify_runtime_lock() -> tuple[dict[str, Any], dict[str, dict[str, Any]], dict[str, Any]]:
    lock = _read_pinned_lock(
        RUNTIME_LOCK_TEXT, PINNED_RUNTIME_LOCK_SHA256, label="V3 runtime lock"
    )
    required = {
...
class VerifiedBytesFinder(importlib.abc.MetaPathFinder):
    """Only loader authorized to create governed V3 Python modules."""

    def __init__(self, source_bytes: Mapping[str, bytes]) -> None:
        self.modules: dict[str, tuple[bytes, str, bool]] = {}
        prefix = "research/model_zoo/causal_valuation_tcn_v3/"
...
def _write_stage(captured: Mapping[str, bytes]) -> None:
    ...
    if stage.parent != outputs or stage.name != ".causal_valuation_tcn_v3_verified_stage":
...
def _controlled_environment(
    *, receipt: Mapping[str, Any], finder: VerifiedBytesFinder
    ...
    environment.update({
        ...
        "CVTCN_V3_LIVE_PROJECT_ROOT": PROJECT_ROOT_TEXT,
        "CVTCN_V3_SOURCE_STAGE": STAGE_TEXT,
        "CVTCN_V3_PREIMPORT_ATTESTATION_SHA256": _sha256_bytes(receipt_text.encode("ascii")),
        "CVTCN_V3_PREIMPORT_RECEIPT_JSON": receipt_text,
        "CVTCN_V3_VERIFIED_MODULE_ORIGINS_JSON": _canonical(finder.origin_records()).decode("ascii"),
    })
    if "CVTCN_V3_TEST_OUTPUT_CONTAINER" in os.environ:
        environment["CVTCN_V3_TEST_OUTPUT_CONTAINER"] = os.environ["CVTCN_V3_TEST_OUTPUT_CONTAINER"]
    return environment
...
def _worker(task: str, device: str | None) -> int:
    ...
    if os.environ.get("CVTCN_V3_PREIMPORT_ATTESTATION_SHA256") != receipt_sha:
        raise PreimportError("parent/worker pre-import attestation differs")
    if os.environ.get("CVTCN_V3_PREIMPORT_RECEIPT_JSON") != receipt_text:
    ...
    if os.environ.get("CVTCN_V3_VERIFIED_MODULE_ORIGINS_JSON") != expected_origins:
    ...
    if task == "tests":
        ...
        relative = "tests/model_lab/causal_valuation_tcn_v3/torch_checks.py"
        namespace = {
            "__name__": "__main__",
...
def _parent(*, preflight: bool) -> int:
    ...
    try:
        ...
        if not preflight:
            output = {
                "schema_version": "expected_pe.causal_valuation_tcn_v3.trusted_tests.v1",
        ...
        output = {
            "schema_version": "expected_pe.causal_valuation_tcn_v3.trusted_preflight.v1",
