"""Stdlib-first exact runtime launcher preflight for future H-OFS V3 inference.
...
def _current_affinity_mask() -> int:
    if os.name != "nt":
        raise RuntimeError("H-OFS V3 launcher requires Windows")
...
def _preimport_guard() -> dict[str, object]:
    ...
    if observed != expected:
        ...
        raise RuntimeError(f"H-OFS V3 pre-import runtime drift: {drifted}")
    return observed
...
def main() -> int:
    ...
    from research.model_zoo.hierarchical_observable_fair_value_state_v3.runtime import (
        capture_runtime_receipt_v3,
    )

    receipt = capture_runtime_receipt_v3(purpose="PREFLIGHT")
    print(
        json.dumps(
            {
