"""Subprocess-only worker for isolated R6 transaction fault/race probes."""
...
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r6.transaction import (
    ...
    execute_live_authority_fixture_transaction,
...
def main() -> int:
    ...
    parser.add_argument("--live-authority", action="store_true")
    ...
    if args.live_authority:
        if args.observed_at_utc is None:
            parser.error("--live-authority requires --observed-at-utc")
        paths = TransactionPaths(
            registry=root / "synthetic_v04_registry.json",
            runtime_root=root / "runtime_r6_authority",
            journal=root / "authority_reservation.journal.json",
            os_lock=root / "synthetic_v04_registry.r6.oslock",
            legacy_lock=root / "synthetic_v04_registry.json.lock",
            fixture_root=root,
        )
        try:
            receipt = execute_live_authority_fixture_transaction(
                paths,
                root / "AUTHORITY_FIXTURE.json",
                observed_at_utc=args.observed_at_utc,
            )
        except R6TransactionError as exc:
    ...
    paths = TransactionPaths(
        ...
        os_lock=root / "fixture_registry.r6.oslock",
