"""Run exact score-free probes and atomically freeze the R6 precommit."""
...
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r6.precommit import (
...
TEST_MAIN = "tests/model_lab/test_observable_state_bce_dgp_tournament_v2_r6.py"
TEST_TRANSACTION = (
...
PROBE_NODEIDS = {
    ...
    "R5_POST_GO_recovery_blocker_supersession_exact": [
        f"{TEST_MAIN}::test_R5_POST_GO_recovery_blocker_supersession_exact"
    ],
    ...
    "live_authority_journal_first_new_and_recovery_exact": [
        (
            f"{TEST_TRANSACTION}::"
            "test_live_authority_journal_first_new_and_recovery_exact"
        )
    ],
    "live_authority_crash_matrix_recovers_exactly_once": [
        (
            f"{TEST_TRANSACTION}::"
            "test_live_authority_crash_matrix_recovers_exactly_once"
        )
    ],
    "post_expiry_durable_plan_completion_and_no_journal_rejection": [
        (
            f"{TEST_TRANSACTION}::"
            "test_post_expiry_durable_plan_completion_and_no_journal_rejection"
        )
    ],
    "journal_absent_after_state_and_corrupt_swapped_authority_rejected": [
        (
            f"{TEST_TRANSACTION}::"
            "test_journal_absent_after_state_and_corrupt_swapped_authority_rejected"
        )
    ],
}
...
def main() -> int:
    if str(Path(sys.argv[0])) != str(SELF) or str(Path(__file__)) != str(SELF):
        raise RuntimeError("R6 freeze must be invoked by its exact absolute path")
    if (
        ...
        raise RuntimeError("R6 freeze interpreter/tool/probe universe differs")
    ...
    ruff_targets = [
        "research/model_zoo/observable_state_bce_dgp_tournament_v2_r6",
        "scripts/model_lab/observable_state_bce_dgp_tournament_v2_r6",
        TEST_MAIN,
        TEST_TRANSACTION,
    ]
    ...
    if registry_after != registry_before:
        raise RuntimeError("registry invariant changed during score-free R6 freeze")
