"""Hard-coded stdlib-only R5 pre-import verified-bytes bootstrap."""
...
PACKAGE_TEXT = PROJECT_TEXT + (
    r"\research\model_zoo\observable_state_bce_dgp_tournament_v2_r5"
)
...
PRECOMMIT_TEXT = PROJECT_TEXT + (
    ...
    r"score_free_precommit_r5_20260821"
)
...
REPARSE = getattr(stat, "FILE_ATTRIBUTE_REPARSE_POINT", 0x400)
stage = Path(os.environ["R5_STAGE_ROOT"])
manifest_path = stage / "EXECUTABLE_MANIFEST.json"
manifest_raw = manifest_path.read_bytes()
if hashlib.sha256(manifest_raw).hexdigest() != os.environ["R5_EXECUTABLE_MANIFEST_RAW_SHA256"]:
...
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r5 import bootstrap_entry
...
def _load_manifest() -> tuple[dict[str, object], bytes]:
    ...
    expected_stage_sources = {
        f"research/model_zoo/observable_state_bce_dgp_tournament_v2_r5/{name}"
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="r5_source_only_") as temporary:
        ...
        environment = {
            ...
            "R5_BOOTSTRAP_COMMAND": args.command,
            "R5_BOOTSTRAP_PARENT_NONCE": secrets.token_hex(32),
            "R5_EXECUTABLE_MANIFEST_RAW_SHA256": hashlib.sha256(manifest_raw).hexdigest(),
        ...
        if args.command == "reserve_authenticated":
            environment["R5_RESERVATION_ACTIVATION"] = RESERVATION_ACTIVATION
