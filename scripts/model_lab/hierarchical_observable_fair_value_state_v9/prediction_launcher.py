"""Formal score-blind H-OFS V9 R4 prediction-only launcher and publisher.

The module is stdlib-only until :func:`preimport_guard` has accepted the exact
runtime.  ``--check`` validates the frozen V9 repair design and its exact V7
...
future independent V9 GO audit, performs the exact inherited block plan, and
...
V9_DESIGN_ROOT = (
    OUTPUTS_ROOT
    / "model_zoo_hierarchical_observable_fair_value_state_v9_"
...
EXPECTED_V9_SOURCE_PATHS = (
    "research/model_zoo/hierarchical_observable_fair_value_state_v9/__init__.py",
    "research/model_zoo/hierarchical_observable_fair_value_state_v9/bootstrap.py",
    "research/model_zoo/hierarchical_observable_fair_value_state_v9/contracts.py",
    "research/model_zoo/hierarchical_observable_fair_value_state_v9/source_audit.py",
    "research/model_zoo/hierarchical_observable_fair_value_state_v9/DESIGN.md",
    "scripts/model_lab/hierarchical_observable_fair_value_state_v9/freeze_design.py",
    LAUNCHER_RELATIVE_PATH,
    "tests/model_lab/test_hierarchical_observable_fair_value_state_v9.py",
    ...
    "scripts/model_lab/hierarchical_observable_fair_value_state_v8/prediction_launcher.py",
...
V9_AUDIT_VERDICT = "GO_SCORE_BLIND_PREDICTION_ONLY_LAUNCH_EXACT_FROZEN_V9_ONLY"
...
def current_affinity_mask() -> int:
    if os.name != "nt":
        raise RuntimeError("H-OFS V9 prediction-only launch requires Windows")
...
def v9_worker_bootstrap_initializer(
    ...
    # This is the one and only absence-style guard in each worker PID.
    guard = preimport_guard()
    from research.model_zoo.hierarchical_observable_fair_value_state_v9.bootstrap import (  # noqa: PLC0415
...
def validate_worker_task_lifecycle(task_position: int) -> dict[str, Any]:
    from research.model_zoo.hierarchical_observable_fair_value_state_v9.bootstrap import (  # noqa: PLC0415
        validate_loaded_worker_closure,
    )

    return validate_loaded_worker_closure(task_position)
...
def launch_contract_payload(
    *,
    ...
    return {
        "schema_version": "expected_pe.hofs_v9.prediction_launch_contract.v1",
        "status": "FROZEN_V9_PREDICTION_ONLY_LAUNCHER_AWAITS_INDEPENDENT_AUDIT",
        "design_contract_sha256": require_sha256(
            design_contract_sha256,
            label="V9 design contract hash",
        ),
        ...
        "required_audit_verdict": V9_AUDIT_VERDICT,
        ...
        "formal_check_worker_lifecycle": {
            "process_start_method": "spawn",
            "exact_initializer_worker_count": 32,
            "production_equivalent_task_count": 50,
            "targeted_same_pid_task_count": 7,
            "initializer_count_per_pid": 1,
            "absence_guard_count_per_pid": 1,
            "every_task_loaded_closure_revalidation": True,
            "first_task_barrier_for_exact_worker_fanout": True,
...
def verify_design_and_public_surfaces(
    *,
    ...
    contents, hashes = require_exact_bundle(
        V9_DESIGN_ROOT,
        universe=V9_DESIGN_UNIVERSE,
        expected_checksums_raw_sha256=design_checksums_raw_sha256,
    )
    manifest = parse_json_exact(contents["MANIFEST.json"], label="V9:MANIFEST")
    if manifest.get("design_contract_sha256") != design_contract_sha256:
        raise RuntimeError("V9 design contract binding drifted")
    if manifest.get("expected_final_file_universe") != list(V9_DESIGN_UNIVERSE):
        raise RuntimeError("V9 design manifest universe drifted")
    from research.model_zoo.hierarchical_observable_fair_value_state_v9.contracts import (  # noqa: PLC0415
        V7_AUDIT_BINDING,
        V7_DESIGN_BINDING,
    ...
    if (
        ...
        != sha256_bytes(canonical_json_bytes(V7_SINGLE_RUN_FAILURE_EVIDENCE))
        or failure_closure.get("v8_design_binding") != V8_DESIGN_BINDING
        or failure_closure.get("v8_audit_binding") != V8_AUDIT_BINDING
        or failure_closure.get("v8_single_run_failure_evidence")
        != V8_SINGLE_RUN_FAILURE_EVIDENCE
        or failure_closure.get("v8_single_run_failure_evidence_sha256")
        != failure_evidence_sha256()
        or failure_closure.get("controller_acknowledged_task_count") != 1
        or failure_closure.get("published_completed_fit_count") != 0
        or failure_closure.get("published_completed_prediction_row_count") != 0
        or failure_closure.get("usable_prediction_row_count") != 0
    ):
        raise RuntimeError("V9 exact V8 single-run worker failure binding drifted")

    source_closure = parse_json_exact(
        contents["SOURCE_CLOSURE.json"],
        label="V9:SOURCE_CLOSURE",
    )
    source_audit = source_closure.get("source_audit")
    if type(source_audit) is not dict or source_closure.get("source_file_count") != len(
        EXPECTED_V9_SOURCE_PATHS
    ):
        raise RuntimeError("V9 source closure count drifted")
    source_map = _strict_pair_map(
        source_audit.get("source_sha256"),
        expected_paths=EXPECTED_V9_SOURCE_PATHS,
    )
    ...
    for relative, expected in source_map.items():
        ...
        if actual != expected:
            raise RuntimeError(f"V9 source byte drifted: {relative}")
    ...
    if source_map.get(LAUNCHER_RELATIVE_PATH) != launcher_sha256:
        raise RuntimeError("V9 source closure does not bind the executing launcher")
    if contents["PREDICTION_LAUNCHER.py"] != launcher_bytes:
        raise RuntimeError("embedded V9 prediction launcher differs from executing bytes")

    launch_contract = parse_json_exact(
        contents["PREDICTION_LAUNCH_CONTRACT.json"],
        label="V9:PREDICTION_LAUNCH_CONTRACT",
    )
    ...
    if unsigned_contract != expected_contract:
        raise RuntimeError("embedded V9 prediction launch contract drifted")

    input_closure = parse_json_exact(
        contents["INPUT_CLOSURE.json"],
        label="V9:INPUT_CLOSURE",
    )
    ...
    if (
        ...
        raise RuntimeError("V9 public input closure drifted")
    if sha256_bytes(canonical_json_bytes(public_files)) != input_binding.get(
        "bound_public_files_sha256"
    ):
        raise RuntimeError("V9 public semantic ledger drifted")
    ...
    snapshot = {
        "v9_design_files": dict(sorted(hashes.items())),
        "v9_and_inherited_source_files": dict(sorted(source_snapshot.items())),
...
def verify_independent_go_audit(
    *,
    ...
    audit = parse_json_exact(contents["AUDIT.json"], label="V9:AUDIT")
    if (
        sha256_bytes(contents["AUDIT.json"])
        != require_sha256(audit_raw_sha256, label="V9 audit raw hash")
        or logical_sha256(audit)
        != require_sha256(audit_semantic_sha256, label="V9 audit semantic hash")
        or audit.get("verdict") != V9_AUDIT_VERDICT
        ...
        raise RuntimeError("V9 independent GO audit drifted")
    seal_receipt = parse_json_exact(contents["SEAL_RECEIPT.json"], label="V9:AUDIT_SEAL")
    if (
        seal_receipt.get("verdict") != V9_AUDIT_VERDICT
        ...
        raise RuntimeError("V9 independent audit authority seal drifted")
    return {
        "hashes": hashes,
        "audit": audit,
        "seal": seal_receipt,
...
def execute_task(
    ...
    worker_lifecycle = validate_worker_task_lifecycle(task_ordinal)
    worker_guard = worker_lifecycle["preimport_guard_receipt"]
    ...
    for spec in plan:
        ...
        block_receipt = sealed(
            {
                "schema_version": "expected_pe.hofs_v9.prediction_block_receipt.v1",
    ...
    task_receipt = sealed(
        {
            "schema_version": "expected_pe.hofs_v9.prediction_task_receipt.v1",
            "task_ordinal": task_ordinal,
            "seed": seed,
            "dgp": dgp,
    ...
    return {
        ...
        "worker_lifecycle_validation": worker_lifecycle,
...
def verify_output_bundle(
    ...
    *,
    ...
    if (
        ...
        != "expected_pe.hofs_v9.prediction_only_root_manifest.v1"
    ...
    if (
        ...
        != "expected_pe.hofs_v9.prediction_output_seal.v1"
    ...
    return {
        "status": "PASS_EXACT_SEALED_V9_PREDICTION_ONLY_OUTPUT_BUNDLE",
...
def report_bytes() -> bytes:
    return (
        "# H-OFS V9 Score-Blind Prediction-Only Execution\n\n"
        "Status: sealed and atomically published prediction-only output.\n\n"
        "The exact frozen V9 design and externally pinned independent GO audit were verified "
...
def _validate_live_v9_surface(
    *,
    ...
    from research.model_zoo.hierarchical_observable_fair_value_state_v9 import (  # noqa: PLC0415
        contract_sha256,
        require_failure_staging,
    )

    if contract_sha256() != design_contract_sha256:
        raise RuntimeError("live V9 contract differs from detached design-contract pin")
    ...
    if any(closure.get(key) != frozen.get(key) for key in compared_keys):
        raise RuntimeError("live public R4 closure differs from frozen V9 input closure")
    ...
    if (
        ...
        raise RuntimeError("live V9 inherited R4 geometry drifted")
    ...
    *,
    ...
    try:
        with ProcessPoolExecutor(
            ...
            mp_context=context,
            initializer=v9_worker_bootstrap_initializer,
            initargs=(
                serialized_task,
                str(PROJECT_ROOT),
            ...
            task_receipts = [future.result(timeout=120.0) for future in futures]
    finally:
        receipt_queue.close()
        receipt_queue.join_thread()
    return validate_lifecycle_task_receipts(
        initializer_receipts,
        task_receipts,
...
def run_worker_bootstrap_check(
    *,
    ...
    from research.model_zoo.hierarchical_observable_fair_value_state_v9.contracts import (  # noqa: PLC0415
        ...
        TARGETED_REUSE_TASK_COUNT,
        bootstrap_contract_sha256,
        sealed_payload,
    )

    if (
        BOOTSTRAP_WORKER_COUNT != PINNED_OUTER_WORKERS
        or BOOTSTRAP_TASK_COUNT != TASK_COUNT
        or TARGETED_REUSE_TASK_COUNT < 2
    ):
        raise RuntimeError("V9 lifecycle/resource geometry cross-binding drifted")
    ast_receipt = validate_execute_task_import_ast(launcher_bytes.decode("utf-8"))
    production_equivalent = _spawn_lifecycle_probe(
        design=design,
        worker_count=BOOTSTRAP_WORKER_COUNT,
        task_count=BOOTSTRAP_TASK_COUNT,
    )
    targeted_same_pid = _spawn_lifecycle_probe(
        design=design,
        worker_count=1,
        task_count=TARGETED_REUSE_TASK_COUNT,
    )
    targeted_counts = targeted_same_pid["task_counts_by_pid"]
    if (
        len(targeted_counts) != 1
        or targeted_counts[0][1] != TARGETED_REUSE_TASK_COUNT
        or targeted_same_pid["maximum_reuse_count"] != TARGETED_REUSE_TASK_COUNT
        or targeted_same_pid["initializer_count_per_pid"] != 1
        or targeted_same_pid["absence_guard_count_per_pid"] != 1
    ):
        raise RuntimeError("V9 targeted same-PID arbitrary reuse proof drifted")
    payload = {
        "schema_version": "expected_pe.hofs_v9.formal_worker_lifecycle.v1",
        "status": "PASS_PRODUCTION_EQUIVALENT_AND_TARGETED_SAME_PID_REUSE",
        "process_start_method": "spawn",
        "bootstrap_contract_sha256": bootstrap_contract_sha256(),
        "execute_task_import_ast": ast_receipt,
        "production_equivalent_50_task_32_worker_schedule": production_equivalent,
        "targeted_same_pid_reuse_schedule": targeted_same_pid,
        "initializer_count_per_pid": 1,
        "absence_guard_count_per_pid": 1,
        "every_task_loaded_closure_validation": True,
        "real_fit_count": 0,
        "real_prediction_count": 0,
...
def check_only(
    *,
    ...
    closure, runtime = _validate_live_v9_surface(
        design=design,
        design_contract_sha256=design_contract_sha256,
    )
    ...
    return {
        "status": "PASS_FORMAL_V9_PREDICTION_ONLY_LAUNCH_CHECK_WITH_32_WORKER_BOOTSTRAP",
...
def run_authorized(
    *,
    ...
    public_closure, launch_runtime = _validate_live_v9_surface(
    ...
    context = multiprocessing.get_context("spawn")
    initializer_barrier = context.Barrier(PINNED_OUTER_WORKERS)
    first_task_barrier = context.Barrier(PINNED_OUTER_WORKERS)
    receipt_queue = context.Queue()
    from research.model_zoo.hierarchical_observable_fair_value_state_v9.bootstrap import (  # noqa: PLC0415
    ...
    serialized_bootstrap_task = canonical_json_bytes(BOOTSTRAP_SAMPLE_TASK)
    production_initializer_receipts: list[dict[str, Any]] = []
    try:
        with ProcessPoolExecutor(
            ...
            initializer=v9_worker_bootstrap_initializer,
            initargs=(
                serialized_bootstrap_task,
                str(PROJECT_ROOT),
                dict(design_before["source_map"]),
                initializer_barrier,
                first_task_barrier,
                receipt_queue,
            ),
            ...
            production_initializer_receipts = [
                receipt_queue.get(timeout=90.0)
                for _ in range(PINNED_OUTER_WORKERS)
            ]
        ...
        receipt_queue.close()
        receipt_queue.join_thread()
    production_initializer_fanout = validate_fanout_receipts(
        production_initializer_receipts,
        expected_worker_count=PINNED_OUTER_WORKERS,
    )
    ...
    production_lifecycle_schedule = validate_lifecycle_task_receipts(
        production_initializer_receipts,
        [result["worker_lifecycle_validation"] for result in ordered_results],
        expected_worker_count=PINNED_OUTER_WORKERS,
        expected_task_count=TASK_COUNT,
    )
    ...
    if design_after["snapshot"] != design_before["snapshot"]:
        raise RuntimeError("source/public/V9 immutable surface changed during launch")
    if require_regular_file(LAUNCHER_PATH, allowed_root=PROJECT_ROOT) != launcher_bytes:
    ...
    input_receipt = sealed(
        {
            "schema_version": "expected_pe.hofs_v9.prediction_input_receipt.v1",
    ...
    immutability_receipt = sealed(
        {
            "schema_version": "expected_pe.hofs_v9.prediction_immutability_receipt.v1",
            "status": "PASS_SOURCE_PUBLIC_V9_BYTES_UNCHANGED_PRE_TO_POST_EXECUTION",
            ...
            "v9_design_checksums_raw_sha256": design_checksums_raw_sha256,
            "v9_audit_checksums_raw_sha256": audit_checksums_raw_sha256,
            "v9_audit_raw_sha256": audit_raw_sha256,
            "v9_audit_semantic_sha256": audit_semantic_sha256,
            "launcher_raw_sha256": sha256_bytes(launcher_bytes),
    ...
    runtime_receipt = sealed(
        {
            "schema_version": "expected_pe.hofs_v9.prediction_runtime_receipt.v1",
            ...
            "production_worker_initializer_fanout": production_initializer_fanout,
            "production_worker_lifecycle_schedule": production_lifecycle_schedule,
    ...
    execution_receipt = sealed(
        {
            "schema_version": "expected_pe.hofs_v9.prediction_execution_receipt.v1",
    ...
    access_receipt = sealed(
        {
            "schema_version": "expected_pe.hofs_v9.prediction_access_receipt.v1",
    ...
    manifest = sealed(
        {
            "schema_version": "expected_pe.hofs_v9.prediction_only_root_manifest.v1",
            "status": "PASS_PREDICTION_ONLY_OUTPUT_READY_FOR_ATOMIC_PUBLISH",
            "final_root": final_relative,
            "v9_design_contract_sha256": design_contract_sha256,
            "v9_design_checksums_raw_sha256": design_checksums_raw_sha256,
            "v9_audit_checksums_raw_sha256": audit_checksums_raw_sha256,
            "v9_audit_raw_sha256": audit["raw_sha256"],
            "v9_audit_semantic_sha256": audit["semantic_sha256"],
    ...
    seal_receipt = sealed(
        {
            "schema_version": "expected_pe.hofs_v9.prediction_output_seal.v1",
    ...
    return {
        **published_verification,
        "status": "SEALED_AND_ATOMICALLY_PUBLISHED_V9_PREDICTION_ONLY_OUTPUT",
        "final_root": final_relative,
...
def main() -> int:
    ...
    design_checksums = require_sha256(
        ...
        label="V9 design checksums raw hash",
    )
    ...
    if args.check:
        ...
        audit_checksums = require_sha256(
            args.audit_checksums_raw_sha256,
            label="V9 audit checksums raw hash",
        )
        audit_raw = require_sha256(args.audit_raw_sha256, label="V9 audit raw hash")
        audit_semantic = require_sha256(
            args.audit_semantic_sha256,
            label="V9 audit semantic hash",
        )
