"""Externally pinned, stdlib-only launcher for the score-free V4 preflight.
...
SOURCE_LOCK_TEXT = PROJECT_ROOT_TEXT + (
    "\\scripts\\model_lab\\causal_valuation_tcn_v4\\V4_SOURCE_LOCK.json"
)
RUNTIME_LOCK_TEXT = PROJECT_ROOT_TEXT + (
    "\\scripts\\model_lab\\causal_valuation_tcn_v4\\V4_RUNTIME_LOCK.json"
)
LAUNCHER_TEXT = PROJECT_ROOT_TEXT + (
    "\\scripts\\model_lab\\causal_valuation_tcn_v4\\trusted_launcher.py"
)
STAGE_TEXT = PROJECT_ROOT_TEXT + "\\outputs\\.causal_valuation_tcn_v4_verified_stage"
...
PINNED_SOURCE_LOCK_SHA256 = "04bb581c479cbb1fa69493c935d577fc5ab014b88508ec05bc71cdc61bd7afbd"
PINNED_RUNTIME_LOCK_SHA256 = "887105ce7c5704be58d91a3424a68e36c30ebd2c0ace09ba2e6c00fc4b3661fc"

SOURCE_SCHEMA = "expected_pe.causal_valuation_tcn_v4.source_lock.v1"
RUNTIME_SCHEMA = "expected_pe.causal_valuation_tcn_v4.runtime_lock.v1"
PREIMPORT_SCHEMA = "expected_pe.causal_valuation_tcn_v4.preimport_attestation.v2"
SOURCE_PACKAGE_PREFIX = "research.model_zoo.causal_valuation_tcn_v4"
...
def _verify_source_lock() -> tuple[dict[str, Any], dict[str, bytes], dict[str, Any]]:
    ...
    lock = _read_pinned_lock(
        SOURCE_LOCK_TEXT, PINNED_SOURCE_LOCK_SHA256, label="V4 source lock"
    )
    required = {
        "schema_version",
        "status",
...
def _distribution_receipt(
    ...
    seen: set[str] = set()
    duplicate_paths = 0
    members = hashlib.sha256(b"expected_pe.causal_valuation_tcn_v4.record_members.v1\0")
    ...
    *,
    ...
    for name in sorted(expected_versions):
        ...
        for key, value in members.items():
            ...
            file_map[key] = value
    closure = hashlib.sha256(b"expected_pe.causal_valuation_tcn_v4.record_closure.v1\0")
...
def _verify_runtime_lock() -> tuple[dict[str, Any], dict[str, dict[str, Any]], dict[str, Any]]:
    lock = _read_pinned_lock(
        RUNTIME_LOCK_TEXT, PINNED_RUNTIME_LOCK_SHA256, label="V4 runtime lock"
    )
    required = {
...
class VerifiedBytesFinder(importlib.abc.MetaPathFinder):
    """Only loader authorized to create governed V4 Python modules."""

    def __init__(self, source_bytes: Mapping[str, bytes]) -> None:
        self.modules: dict[str, tuple[bytes, str, bool]] = {}
        prefix = "research/model_zoo/causal_valuation_tcn_v4/"
...
def _write_stage(captured: Mapping[str, bytes]) -> None:
    ...
    if stage.parent != outputs or stage.name != ".causal_valuation_tcn_v4_verified_stage":
...
def _controlled_environment(
    *, receipt: Mapping[str, Any], finder: VerifiedBytesFinder
    ...
    environment.update({
        ...
        "CVTCN_V4_LIVE_PROJECT_ROOT": PROJECT_ROOT_TEXT,
        "CVTCN_V4_SOURCE_STAGE": STAGE_TEXT,
        "CVTCN_V4_PREIMPORT_ATTESTATION_SHA256": _sha256_bytes(receipt_text.encode("ascii")),
        "CVTCN_V4_PREIMPORT_RECEIPT_JSON": receipt_text,
        "CVTCN_V4_VERIFIED_MODULE_ORIGINS_JSON": _canonical(finder.origin_records()).decode("ascii"),
    })
    if "CVTCN_V4_TEST_OUTPUT_CONTAINER" in os.environ:
        environment["CVTCN_V4_TEST_OUTPUT_CONTAINER"] = os.environ["CVTCN_V4_TEST_OUTPUT_CONTAINER"]
    return environment
...
def _worker(task: str, device: str | None) -> int:
    ...
    if os.environ.get("CVTCN_V4_PREIMPORT_ATTESTATION_SHA256") != receipt_sha:
        raise PreimportError("parent/worker pre-import attestation differs")
    if os.environ.get("CVTCN_V4_PREIMPORT_RECEIPT_JSON") != receipt_text:
    ...
    if os.environ.get("CVTCN_V4_VERIFIED_MODULE_ORIGINS_JSON") != expected_origins:
    ...
    if task == "tests":
        ...
        relative = "tests/model_lab/causal_valuation_tcn_v4/torch_checks.py"
        namespace = {
            "__name__": "__main__",
...
def _parent(*, preflight: bool) -> int:
    ...
    try:
        ...
        if not preflight:
            output = {
                "schema_version": "expected_pe.causal_valuation_tcn_v4.trusted_tests.v1",
        ...
        output = {
            "schema_version": "expected_pe.causal_valuation_tcn_v4.trusted_preflight.v1",
