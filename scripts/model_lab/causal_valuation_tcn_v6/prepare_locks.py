"""One-time deterministic preparation of the externally pinned V6 lock files.

This script is not part of the trusted launch path.  It inventories only code
and environment metadata already authorized for the score-free V6 repair.
...
def _runtime_lock() -> dict[str, Any]:
    ...
    return {
        "schema_version": "expected_pe.causal_valuation_tcn_v6.runtime_lock.v1",
...
def _source_lock() -> dict[str, Any]:
    source_paths = sorted([
        *(f"research/model_zoo/causal_valuation_tcn_v6/{name}" for name in PACKAGE_FILES),
        "scripts/model_lab/causal_valuation_tcn_v6/build_preflight.py",
        "scripts/model_lab/causal_valuation_tcn_v6/prepare_locks.py",
        "tests/model_lab/causal_valuation_tcn_v6/torch_checks.py",
    ])
    ...
    return {
        "schema_version": "expected_pe.causal_valuation_tcn_v6.source_lock.v1",
        "status": "EXTERNALLY_PINNED_VERIFIED_SOURCE_BYTES",
        "project_root": str(PROJECT_ROOT).replace("/", "\\"),
        "source_records": records,
        "exact_roots": {
            "research/model_zoo/causal_valuation_tcn_v6": {
                name: "file" for name in sorted(PACKAGE_FILES)
            },
            "scripts/model_lab/causal_valuation_tcn_v6": {
                name: "file" for name in (
                    "V6_RUNTIME_LOCK.json", "V6_SOURCE_LOCK.json", "build_preflight.py",
                    "prepare_locks.py", "trusted_launcher.py",
                )
            },
            "tests/model_lab/causal_valuation_tcn_v6": {"torch_checks.py": "file"},
        },
