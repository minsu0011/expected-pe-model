"""Externally pinned, stdlib-only launcher for the score-free V6 preflight.
...
SOURCE_LOCK_TEXT = PROJECT_ROOT_TEXT + (
    "\\scripts\\model_lab\\causal_valuation_tcn_v6\\V6_SOURCE_LOCK.json"
)
RUNTIME_LOCK_TEXT = PROJECT_ROOT_TEXT + (
    "\\scripts\\model_lab\\causal_valuation_tcn_v6\\V6_RUNTIME_LOCK.json"
)
LAUNCHER_TEXT = PROJECT_ROOT_TEXT + (
    "\\scripts\\model_lab\\causal_valuation_tcn_v6\\trusted_launcher.py"
)
STAGE_TEXT = PROJECT_ROOT_TEXT + "\\outputs\\.causal_valuation_tcn_v6_verified_stage"
...
PINNED_SOURCE_LOCK_SHA256 = "136fdffaee96fe9da550c529b49847a838a63cbfa6f34ef1a75e3d8c54b65e04"
PINNED_RUNTIME_LOCK_SHA256 = "d997e9ff0152826649c7b67e34279b7245f51b80afff38e06c22ff4d73fe1f2a"

SOURCE_SCHEMA = "expected_pe.causal_valuation_tcn_v6.source_lock.v1"
RUNTIME_SCHEMA = "expected_pe.causal_valuation_tcn_v6.runtime_lock.v1"
PREIMPORT_SCHEMA = "expected_pe.causal_valuation_tcn_v6.preimport_attestation.v2"
SOURCE_PACKAGE_PREFIX = "research.model_zoo.causal_valuation_tcn_v6"
...
def _verify_source_lock() -> tuple[dict[str, Any], dict[str, bytes], dict[str, Any]]:
    ...
    lock = _read_pinned_lock(
        SOURCE_LOCK_TEXT, PINNED_SOURCE_LOCK_SHA256, label="V6 source lock"
    )
    required = {
        "schema_version",
        "status",
...
def _distribution_receipt(
    ...
    seen: set[str] = set()
    duplicate_paths = 0
    members = hashlib.sha256(b"expected_pe.causal_valuation_tcn_v6.record_members.v1\0")
    ...
    *,
    ...
    for name in sorted(expected_versions):
        ...
        for key, value in members.items():
            ...
            file_map[key] = value
    closure = hashlib.sha256(b"expected_pe.causal_valuation_tcn_v6.record_closure.v1\0")
...
def _verify_runtime_lock() -> tuple[dict[str, Any], dict[str, dict[str, Any]], dict[str, Any]]:
    lock = _read_pinned_lock(
        RUNTIME_LOCK_TEXT, PINNED_RUNTIME_LOCK_SHA256, label="V6 runtime lock"
    )
    required = {
...
class VerifiedBytesFinder(importlib.abc.MetaPathFinder):
    """Only loader authorized to create governed V6 Python modules."""

    def __init__(self, source_bytes: Mapping[str, bytes]) -> None:
        self.modules: dict[str, tuple[bytes, str, bool]] = {}
        prefix = "research/model_zoo/causal_valuation_tcn_v6/"
...
def _write_stage(captured: Mapping[str, bytes]) -> None:
    ...
    if stage.parent != outputs or stage.name != ".causal_valuation_tcn_v6_verified_stage":
...
def _controlled_environment(
    *, receipt: Mapping[str, Any], finder: VerifiedBytesFinder
    ...
    environment.update({
        ...
        "CVTCN_V6_LIVE_PROJECT_ROOT": PROJECT_ROOT_TEXT,
        "CVTCN_V6_SOURCE_STAGE": STAGE_TEXT,
        "CVTCN_V6_PREIMPORT_ATTESTATION_SHA256": _sha256_bytes(receipt_text.encode("ascii")),
        "CVTCN_V6_PREIMPORT_RECEIPT_JSON": receipt_text,
        "CVTCN_V6_VERIFIED_MODULE_ORIGINS_JSON": _canonical(finder.origin_records()).decode("ascii"),
    })
    if "CVTCN_V6_TEST_OUTPUT_CONTAINER" in os.environ:
        environment["CVTCN_V6_TEST_OUTPUT_CONTAINER"] = os.environ["CVTCN_V6_TEST_OUTPUT_CONTAINER"]
    return environment
...
def _worker(task: str, device: str | None) -> int:
    ...
    if os.environ.get("CVTCN_V6_PREIMPORT_ATTESTATION_SHA256") != receipt_sha:
        raise PreimportError("parent/worker pre-import attestation differs")
    if os.environ.get("CVTCN_V6_PREIMPORT_RECEIPT_JSON") != receipt_text:
    ...
    if os.environ.get("CVTCN_V6_VERIFIED_MODULE_ORIGINS_JSON") != expected_origins:
    ...
    if task == "tests":
        ...
        relative = "tests/model_lab/causal_valuation_tcn_v6/torch_checks.py"
        namespace = {
            "__name__": "__main__",
...
def _parent(*, preflight: bool) -> int:
    ...
    try:
        ...
        if not preflight:
            output = {
                "schema_version": "expected_pe.causal_valuation_tcn_v6.trusted_tests.v1",
        ...
        output = {
            "schema_version": "expected_pe.causal_valuation_tcn_v6.trusted_preflight.v1",
