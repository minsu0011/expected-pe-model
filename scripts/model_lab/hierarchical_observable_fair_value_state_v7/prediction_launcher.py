"""Formal score-blind H-OFS V7 R4 prediction-only launcher and publisher.

The module is stdlib-only until :func:`preimport_guard` has accepted the exact
runtime.  ``--check`` validates the frozen V7 design, source closure, launch
contract, public R4 closure, and runtime without fitting or predicting.
``--run`` additionally requires detached pins for a future independent V7 GO
audit, performs the exact frozen block plan, and publishes a verified output
root with one same-parent atomic rename.
...
V7_DESIGN_ROOT = (
    OUTPUTS_ROOT
    / "model_zoo_hierarchical_observable_fair_value_state_v7_"
...
V7_AUDIT_VERDICT = "GO_SCORE_BLIND_PREDICTION_ONLY_LAUNCH_EXACT_FROZEN_V7_ONLY"
...
def current_affinity_mask() -> int:
    if os.name != "nt":
        raise RuntimeError("H-OFS V7 prediction-only launch requires Windows")
...
def launch_contract_payload(
    *,
    ...
    return {
        ...
        "required_audit_verdict": V7_AUDIT_VERDICT,
...
def verify_design_and_public_surfaces(
    *,
    ...
    contents, hashes = require_exact_bundle(
        V7_DESIGN_ROOT,
        universe=V7_DESIGN_UNIVERSE,
        expected_checksums_raw_sha256=design_checksums_raw_sha256,
    )
    manifest = parse_json_exact(contents["MANIFEST.json"], label="V7:MANIFEST")
    if manifest.get("design_contract_sha256") != design_contract_sha256:
        raise RuntimeError("V7 design contract binding drifted")
    if manifest.get("expected_final_file_universe") != list(V7_DESIGN_UNIVERSE):
        raise RuntimeError("V7 design manifest universe drifted")

    source_closure = parse_json_exact(
        contents["SOURCE_CLOSURE.json"],
        label="V7:SOURCE_CLOSURE",
    )
    source_audit = source_closure.get("source_audit")
    if type(source_audit) is not dict or source_closure.get("source_file_count") != len(
        EXPECTED_V7_SOURCE_PATHS
    ):
        raise RuntimeError("V7 source closure count drifted")
    source_map = _strict_pair_map(
        source_audit.get("source_sha256"),
        expected_paths=EXPECTED_V7_SOURCE_PATHS,
    )
    ...
    for relative, expected in source_map.items():
        ...
        if actual != expected:
            raise RuntimeError(f"V7 source byte drifted: {relative}")
    ...
    if source_map.get(LAUNCHER_RELATIVE_PATH) != launcher_sha256:
        raise RuntimeError("V7 source closure does not bind the executing launcher")
    if contents["PREDICTION_LAUNCHER.py"] != launcher_bytes:
        raise RuntimeError("embedded V7 prediction launcher differs from executing bytes")

    launch_contract = parse_json_exact(
        contents["PREDICTION_LAUNCH_CONTRACT.json"],
        label="V7:PREDICTION_LAUNCH_CONTRACT",
    )
    ...
    if unsigned_contract != expected_contract:
        raise RuntimeError("embedded V7 prediction launch contract drifted")

    input_closure = parse_json_exact(
        contents["INPUT_CLOSURE.json"],
        label="V7:INPUT_CLOSURE",
    )
    ...
    if (
        ...
        raise RuntimeError("V7 public input closure drifted")
    if sha256_bytes(canonical_json_bytes(public_files)) != input_binding.get(
        "bound_public_files_sha256"
    ):
        raise RuntimeError("V7 public semantic ledger drifted")
    ...
    snapshot = {
        "v7_design_files": dict(sorted(hashes.items())),
        "v7_source_files": dict(sorted(source_snapshot.items())),
...
def verify_independent_go_audit(
    *,
    ...
    contents, hashes = require_exact_bundle(
        V7_AUDIT_ROOT,
        universe=V7_AUDIT_UNIVERSE,
        expected_checksums_raw_sha256=audit_checksums_raw_sha256,
    )
    audit = parse_json_exact(contents["AUDIT.json"], label="V7:AUDIT")
    if (
        sha256_bytes(contents["AUDIT.json"])
        != require_sha256(audit_raw_sha256, label="V7 audit raw hash")
        or logical_sha256(audit)
        != require_sha256(audit_semantic_sha256, label="V7 audit semantic hash")
        or audit.get("verdict") != V7_AUDIT_VERDICT
        ...
        raise RuntimeError("V7 independent GO audit drifted")
    seal_receipt = parse_json_exact(contents["SEAL_RECEIPT.json"], label="V7:AUDIT_SEAL")
    if (
        seal_receipt.get("verdict") != V7_AUDIT_VERDICT
        ...
        raise RuntimeError("V7 independent audit authority seal drifted")
    return {
        "hashes": hashes,
        "audit": audit,
        "seal": seal_receipt,
...
def execute_task(
    ...
    for spec in plan:
        ...
        block_receipt = sealed(
            {
                "schema_version": "expected_pe.hofs_v7.prediction_block_receipt.v1",
    ...
    task_receipt = sealed(
        {
            "schema_version": "expected_pe.hofs_v7.prediction_task_receipt.v1",
            "task_ordinal": task_ordinal,
            "seed": seed,
            "dgp": dgp,
...
def verify_output_bundle(
    ...
    *,
    ...
    if (
        ...
        != "expected_pe.hofs_v7.prediction_only_root_manifest.v1"
    ...
    if (
        ...
        != "expected_pe.hofs_v7.prediction_output_seal.v1"
    ...
    return {
        "status": "PASS_EXACT_SEALED_V7_PREDICTION_ONLY_OUTPUT_BUNDLE",
...
def report_bytes() -> bytes:
    return (
        "# H-OFS V7 Score-Blind Prediction-Only Execution\n\n"
        "Status: sealed and atomically published prediction-only output.\n\n"
        "The exact frozen V7 design and externally pinned independent GO audit were verified "
...
def _validate_live_v7_surface(
    *,
    ...
    if contract_sha256() != design_contract_sha256:
        raise RuntimeError("live V7 contract differs from detached design-contract pin")
    ...
    if any(closure.get(key) != frozen.get(key) for key in compared_keys):
        raise RuntimeError("live public R4 closure differs from frozen V7 input closure")
    ...
    if (
        ...
        raise RuntimeError("live V7 R4 geometry drifted")
...
def check_only(
    *,
    ...
    closure, runtime = _validate_live_v7_surface(
        design=design,
        design_contract_sha256=design_contract_sha256,
    )
    ...
    return {
        "status": "PASS_FORMAL_V7_PREDICTION_ONLY_LAUNCH_CHECK_NO_FIT_NO_PREDICTION",
...
def run_authorized(
    *,
    ...
    public_closure, launch_runtime = _validate_live_v7_surface(
    ...
    if design_after["snapshot"] != design_before["snapshot"]:
        raise RuntimeError("source/public/V7 immutable surface changed during launch")
    if require_regular_file(LAUNCHER_PATH, allowed_root=PROJECT_ROOT) != launcher_bytes:
    ...
    input_receipt = sealed(
        {
            "schema_version": "expected_pe.hofs_v7.prediction_input_receipt.v1",
    ...
    immutability_receipt = sealed(
        {
            "schema_version": "expected_pe.hofs_v7.prediction_immutability_receipt.v1",
            "status": "PASS_SOURCE_PUBLIC_V7_BYTES_UNCHANGED_PRE_TO_POST_EXECUTION",
            ...
            "v7_design_checksums_raw_sha256": design_checksums_raw_sha256,
            "v7_audit_checksums_raw_sha256": audit_checksums_raw_sha256,
            "v7_audit_raw_sha256": audit_raw_sha256,
            "v7_audit_semantic_sha256": audit_semantic_sha256,
            "launcher_raw_sha256": sha256_bytes(launcher_bytes),
    ...
    runtime_receipt = sealed(
        {
            "schema_version": "expected_pe.hofs_v7.prediction_runtime_receipt.v1",
    ...
    execution_receipt = sealed(
        {
            "schema_version": "expected_pe.hofs_v7.prediction_execution_receipt.v1",
    ...
    access_receipt = sealed(
        {
            "schema_version": "expected_pe.hofs_v7.prediction_access_receipt.v1",
    ...
    manifest = sealed(
        {
            "schema_version": "expected_pe.hofs_v7.prediction_only_root_manifest.v1",
            "status": "PASS_PREDICTION_ONLY_OUTPUT_READY_FOR_ATOMIC_PUBLISH",
            "final_root": final_relative,
            "v7_design_contract_sha256": design_contract_sha256,
            "v7_design_checksums_raw_sha256": design_checksums_raw_sha256,
            "v7_audit_checksums_raw_sha256": audit_checksums_raw_sha256,
            "v7_audit_raw_sha256": audit["raw_sha256"],
            "v7_audit_semantic_sha256": audit["semantic_sha256"],
    ...
    seal_receipt = sealed(
        {
            "schema_version": "expected_pe.hofs_v7.prediction_output_seal.v1",
    ...
    return {
        **published_verification,
        "status": "SEALED_AND_ATOMICALLY_PUBLISHED_V7_PREDICTION_ONLY_OUTPUT",
        "final_root": final_relative,
...
def main() -> int:
    ...
    design_checksums = require_sha256(
        ...
        label="V7 design checksums raw hash",
    )
    ...
    if args.check:
        ...
        audit_checksums = require_sha256(
            args.audit_checksums_raw_sha256,
            label="V7 audit checksums raw hash",
        )
        audit_raw = require_sha256(args.audit_raw_sha256, label="V7 audit raw hash")
        audit_semantic = require_sha256(
            args.audit_semantic_sha256,
            label="V7 audit semantic hash",
        )
