"""Stdlib-first exact runtime launcher preflight for future H-OFS V7 inference.
...
_PYTHON_EXECUTABLE_SHA256 = "2de63ce9ee584123173a0c96e68b54be74f7ad17c21a343e63ac12a4790cb21d"
...
def _current_affinity_mask() -> int:
    if os.name != "nt":
        raise RuntimeError("H-OFS V7 launcher requires Windows")
...
def _preimport_guard() -> dict[str, object]:
    ...
    observed = {
        ...
        "thread_environment": [[name, os.environ.get(name, "")] for name, _ in _THREAD_ENVIRONMENT],
        "gpu_environment": [[name, os.environ.get(name, "")] for name, _ in _GPU_ENVIRONMENT],
    }
    expected = {
    ...
    if observed != expected:
        ...
        raise RuntimeError(f"H-OFS V7 pre-import runtime drift: {drifted}")
    return observed
...
def main() -> int:
    ...
    from research.model_zoo.hierarchical_observable_fair_value_state_v7.runtime import (
    ...
    print(
        json.dumps(
            {
                "status": "PASS_EXACT_HOFS_V7_INFERENCE_LAUNCHER_PREFLIGHT_ONLY",
                "preimport": preimport,
