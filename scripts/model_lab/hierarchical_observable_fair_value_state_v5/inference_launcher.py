"""Stdlib-first exact runtime launcher preflight for future H-OFS V5 inference.
...
def _current_affinity_mask() -> int:
    if os.name != "nt":
        raise RuntimeError("H-OFS V5 launcher requires Windows")
...
def _preimport_guard() -> dict[str, object]:
    ...
    if observed != expected:
        ...
        raise RuntimeError(f"H-OFS V5 pre-import runtime drift: {drifted}")
    return observed
...
def main() -> int:
    ...
    from research.model_zoo.hierarchical_observable_fair_value_state_v5.runtime import (
    ...
    print(
        json.dumps(
            {
                "status": "PASS_EXACT_HOFS_V5_INFERENCE_LAUNCHER_PREFLIGHT_ONLY",
                "preimport": preimport,
