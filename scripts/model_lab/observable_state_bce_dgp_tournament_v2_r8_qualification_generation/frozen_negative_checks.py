"""Run frozen R8-r4 fail-closed probes without creating any payload authority."""
...
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r8_qualification_generation.capability import (  # noqa: E402
    CAPABILITY_ENV,
    SECURITY_ATTRIBUTES,
...
LAUNCHER = PROJECT_ROOT / (
    "scripts/model_lab/observable_state_bce_dgp_tournament_v2_r8_"
    "qualification_generation/external_launcher.py"
)
...
NEGATIVE_ROOT = PROJECT_ROOT / (
    ...
    "r8_builder_frozen_negative_checks_r4_20260821"
)
RUN_ID = "20260821T235959"
TOKEN = "de" * 32
...
def _unsigned_authority(anchor: Mapping[str, Any]) -> Mapping[str, Any]:
    now = datetime.now(timezone.utc)
    return {
        "schema_version": "expected_pe.r8.qualification.signed_authority.v1",
        ...
        "audit_root_relative": (
            ...
            "independent_qualification_pre_generation_audit_unsigned_r4_probe"
        ),
        "audit_json_raw_sha256": "1" * 64,
        "audit_seal_raw_sha256": "2" * 64,
        "audit_signer_key_id": AUDITOR_KEY_ID,
...
def main() -> int:
    ...
    if any(path.exists() for path in (NEGATIVE_ROOT, staging)):
        raise RuntimeError("R8-r4 frozen negative-check root already exists")
    if not anchor_path.is_file() or not RUNTIME_PYTHON.is_file():
        raise RuntimeError("R8-r4 freeze/expected runtime is absent")
    anchor_raw = anchor_path.read_bytes()
    anchor = json.loads(anchor_raw)
    if canonical_json_bytes(anchor) != anchor_raw:
        raise RuntimeError("R8-r4 external anchor is not canonical")
    ...
    if sha256_bytes(design_checksums) != anchor["design_checksums_raw_sha256"]:
        raise RuntimeError("R8-r4 frozen design checksum anchor drifted")
    ...
    authority_path = staging / "UNSIGNED_AUTHORITY_PROBE.json"
    _write(authority_path, canonical_json_bytes(_unsigned_authority(anchor)))
    checks.append(
        _run_failure(
            "UNSIGNED_EXACT_KEY_AUTHORITY",
            [
                *_isolated_command(),
                "--authority",
                str(authority_path),
            ...
            environment=environment,
            expected_failure_stage="TOP_LEVEL_AUTHORITY_AND_ACTIVATION",
            stdin=(TOKEN + "\n").encode("ascii"),
        )
    )
    ...
    payload = {
        "schema_version": "expected_pe.r8.r4.builder_frozen_negative_checks.v1",
        "status": "PASS_ALL_R8_R4_FROZEN_NEGATIVE_AND_OS_HANDLE_CHECKS",
        "design_checksums_raw_sha256": anchor["design_checksums_raw_sha256"],
    ...
    report = (
        "# R8-r4 Frozen Negative and Win32 Handle Checks\n\n"
