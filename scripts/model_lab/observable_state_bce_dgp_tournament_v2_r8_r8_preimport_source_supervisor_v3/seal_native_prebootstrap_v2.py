"""One-shot DACL seal for the independently audited managed bootstrap V2."""
...
SUPERVISOR = Path(__file__).resolve().parent / "trusted_supervisor_v4.py"
EXECUTABLE = PROJECT_ROOT / (
    "build/r8r8_native_prebootstrap_v2_immutable_20260823/ExpectedPeNativePrebootstrapV2.exe"
)
EXECUTABLE_SHA256 = "46006c20f762fae971c84d9c08de6c234f5640f35ea5eb08bd43f74b7fe90ea4"
EXECUTABLE_SIZE = 33280
EXECUTABLE_VOLUME = 13325047249941796650
EXECUTABLE_FILE_ID = "0d0f2100000032000000000000000000"
RECEIPT = PROJECT_ROOT / (
    "outputs/r8r8_native_prebootstrap_v2_immutable_seal_receipt_20260823.json"
)
EXACT_ARGUMENT = "--seal-fixed-native-prebootstrap-v2-owner-rights-read-execute-once"
ZERO_COUNTS = {
    "authority": 0,
    "fresh": 0,
    "generation": 0,
...
def _load_supervisor() -> Any:
    ...
    if (
        ...
        != "5266f4864976a11cb1d2c940657e3c87e1824e2b234a229ffea2bbecb86b468c"
    ):
        raise RuntimeError("V4 supervisor drifted before native seal")
    spec = importlib.util.spec_from_file_location("native_v2_seal_v4", SUPERVISOR)
    if spec is None or spec.loader is None:
        raise RuntimeError("V4 supervisor loader is unavailable")
...
def _open_file_for_dacl(module: Any, path: Path) -> int:
    ...
    if handle == module.INVALID_HANDLE_VALUE:
        raise RuntimeError(f"native executable DACL open failed: {ctypes.get_last_error()}")
    return int(handle)


def main(arguments: list[str]) -> int:
    if arguments != [EXACT_ARGUMENT]:
        raise RuntimeError("exact native seal argument is required")
    if RECEIPT.exists():
        raise RuntimeError("native seal receipt identity already exists")
    raw = EXECUTABLE.read_bytes()
    if len(raw) != EXECUTABLE_SIZE or hashlib.sha256(raw).hexdigest() != EXECUTABLE_SHA256:
        raise RuntimeError("native executable bytes drifted before seal")
    ...
    try:
        if module._identity(file_handle) != (EXECUTABLE_VOLUME, EXECUTABLE_FILE_ID):
            raise RuntimeError("native executable identity drifted before seal")
        ...
        if (
            ...
            raise RuntimeError("native immutable DACL fingerprint drifted")
        receipt = {
            ...
            "schema_version": "expected_pe.r8.r8.native_prebootstrap.immutable_seal.v2",
            "status": "PASS_NATIVE_BOOTSTRAP_EXECUTABLE_AND_DIRECTORY_IMMUTABLY_SEALED",
        }
        encoded = json.dumps(
            receipt,
            ensure_ascii=True,
