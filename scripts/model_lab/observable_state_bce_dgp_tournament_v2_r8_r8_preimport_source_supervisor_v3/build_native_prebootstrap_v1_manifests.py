...
V3_ROOT = PROJECT_ROOT / (
    "scripts/model_lab/observable_state_bce_dgp_tournament_v2_r8_r8_preimport_source_supervisor_v3"
)
SOURCE_MANIFEST = V3_ROOT / "SOURCE_CLOSURE_MANIFEST.json"
RUNTIME_MANIFEST = V3_ROOT / "STATIC_CHILD_RUNTIME_CLOSURE.json"
...
FILE_OUTPUT = Path(__file__).resolve().parent / "NATIVE_FILE_CLOSURE.tsv"
DIRECTORY_OUTPUT = Path(__file__).resolve().parent / "NATIVE_DIRECTORY_CLOSURE.tsv"

SOURCE_MANIFEST_SHA256 = "09848b168eb261fd0b6e3de8944ef2c96cb314e33f2b0ae92b26066e533215c9"
...
def _file_bytes() -> bytes:
    ...
    for row in runtime.get("runtime_records", ()):
        ...
        records[key] = value
    support = (
        ("V3_SOURCE_MANIFEST", SOURCE_MANIFEST),
        ("V3_RUNTIME_MANIFEST", RUNTIME_MANIFEST),
        ("V3_STUB_TEMPLATE", V3_ROOT / "STDLIB_LAUNCH_STUB_TEMPLATE.txt"),
        ("V3_SUPERVISOR", V3_ROOT / "trusted_supervisor.py"),
    )
    for role, target in support:
        ...
        records[key] = value
    rows: list[str] = ["EXPECTED_PE_NATIVE_FILE_CLOSURE_V1"]
...
def _directory_bytes() -> bytes:
    ...
    rows = ["EXPECTED_PE_NATIVE_DIRECTORY_CLOSURE_V1"]
    for row in sorted(
        records,
...
def main() -> int:
    ...
    parser.add_argument(
        "--write-fixed-native-closure-v1-no-execution",
        action="store_true",
        required=True,
    )
    arguments = parser.parse_args()
    if arguments.write_fixed_native_closure_v1_no_execution is not True:
