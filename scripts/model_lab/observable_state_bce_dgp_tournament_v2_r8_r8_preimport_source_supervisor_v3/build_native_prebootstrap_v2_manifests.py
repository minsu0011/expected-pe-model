...
SOURCE_MANIFEST = V4_ROOT / "SOURCE_CLOSURE_MANIFEST_V4.json"
RUNTIME_MANIFEST = V4_ROOT / "STATIC_CHILD_RUNTIME_CLOSURE.json"
...
FILE_OUTPUT = Path(__file__).resolve().parent / "NATIVE_FILE_CLOSURE_V2.tsv"
DIRECTORY_OUTPUT = Path(__file__).resolve().parent / "NATIVE_DIRECTORY_CLOSURE_V2.tsv"

SOURCE_MANIFEST_SHA256 = "c4ab727356765c698dcdcbc4ac669780c52f6a2888b92c7a3e122be341d5a21f"
...
def _file_bytes() -> bytes:
    ...
    for row in runtime.get("runtime_records", ()):
        ...
        records[key] = value
    support = (
        ("V4_SOURCE_MANIFEST", SOURCE_MANIFEST),
        ("V4_RUNTIME_MANIFEST", RUNTIME_MANIFEST),
        ("V4_STUB_TEMPLATE", V4_ROOT / "STDLIB_LAUNCH_STUB_TEMPLATE_V4.txt"),
        ("V4_SUPERVISOR", V4_ROOT / "trusted_supervisor_v4.py"),
    )
    for role, target in support:
        ...
        records[key] = value
    rows: list[str] = ["EXPECTED_PE_NATIVE_FILE_CLOSURE_V2"]
...
def _directory_bytes() -> bytes:
    ...
    rows = ["EXPECTED_PE_NATIVE_DIRECTORY_CLOSURE_V2"]
    for row in sorted(
        records,
...
def main() -> int:
    ...
    parser.add_argument(
        "--write-fixed-native-closure-v2-no-execution",
        action="store_true",
        required=True,
    )
    arguments = parser.parse_args()
    if arguments.write_fixed_native_closure_v2_no_execution is not True:
