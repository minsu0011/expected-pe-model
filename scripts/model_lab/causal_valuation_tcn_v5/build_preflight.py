"""Build and freeze the isolated score-free Causal Valuation TCN V5 preflight."""
...
FINAL_OUTPUT = OUTPUTS_ROOT / (
    "model_zoo_causal_valuation_tcn_v5_"
    "score_free_design_environment_preflight_20260821"
)
STAGE = OUTPUTS_ROOT / ".causal_valuation_tcn_v5_verified_stage"
BUNDLE_FILES = (
    "ACCESS_RECEIPT.json",
    "CHECKSUMS.sha256",
    "DESIGN.md",
...
class V5BuildError(RuntimeError):
...
def _ordinary(path: Path, *, directory: bool, label: str) -> Path:
    ...
    for part in absolute.parts[1:]:
        ...
        try:
            ...
            raise V5BuildError(f"{label} is absent") from error
        if stat.S_ISLNK(metadata.st_mode) or bool(
            int(getattr(metadata, "st_file_attributes", 0)) & REPARSE_ATTRIBUTE
        ):
            raise V5BuildError(f"{label} contains a reparse/symlink component")
    metadata = os.lstat(absolute)
    if directory and not stat.S_ISDIR(metadata.st_mode):
        raise V5BuildError(f"{label} is not an ordinary directory")
    if not directory and not stat.S_ISREG(metadata.st_mode):
        raise V5BuildError(f"{label} is not an ordinary file")
    return absolute
...
def _semantic(payload: Mapping[str, Any]) -> dict[str, Any]:
    if "semantic_sha256" in payload:
        raise V5BuildError("payload already contains a semantic seal")
    output = dict(payload)
...
def _run(command: list[str], *, timeout: int, environment: Mapping[str, str]) -> dict[str, Any]:
    ...
    try:
        ...
        raise V5BuildError("subprocess stdout is not strict UTF-8") from error
    return {
        "command": command,
...
def _strict_single_object(text: str, *, label: str) -> dict[str, Any]:
    try:
        ...
        raise V5BuildError(f"{label} did not emit one JSON object") from error
    if type(payload) is not dict:
        raise V5BuildError(f"{label} root is not an object")
    return payload
...
def _launcher_hashes() -> tuple[str, str]:
    ...
    if source_match is None or runtime_match is None:
        raise V5BuildError("trusted launcher lock hashes are not finalized literals")
    if _sha256_file(SOURCE_LOCK) != source_match.group(1):
        raise V5BuildError("source lock differs from trusted launcher")
    if _sha256_file(RUNTIME_LOCK) != runtime_match.group(1):
        raise V5BuildError("runtime lock differs from trusted launcher")
...
def _trusted_preflight() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if result["return_code"] != 0:
        raise V5BuildError("trusted preflight failed:\n" + result["output_tail"])
    ...
    if (
        ...
        raise V5BuildError("trusted preflight receipt drifted")
...
def _test_receipt(preflight: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if type(text) is not str:
        raise V5BuildError("trusted test stdout is absent")
    ...
    if receipt != {
        ...
        raise V5BuildError("trusted adversarial test receipt drifted")
    return receipt
...
def _validate_smoke(preflight: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if type(workers) is not dict or set(workers) != {
        ...
        raise V5BuildError("trusted worker universe drifted")
    boundary = {
    ...
    for name, worker in workers.items():
        if (
            ...
            raise V5BuildError(f"trusted smoke worker drifted: {name}")
        ...
        if (
            ...
            raise V5BuildError("end-to-end nuisance smoke drifted")
    first = workers["gpu_process_1"]
    second = workers["gpu_process_2"]
    if first["reproducibility_digest"] != second["reproducibility_digest"]:
        raise V5BuildError("independent GPU reproducibility digest drifted")
    for field in ("module_origin_records", "module_origin_closure_sha256"):
        if first["runtime_receipt"][field] != second["runtime_receipt"][field]:
            raise V5BuildError(f"independent GPU runtime closure drifted: {field}")
    ...
    for name, worker in (("gpu_process_1", first), ("gpu_process_2", second)):
        ...
        if type(records) is not list:
            raise V5BuildError(f"{name} native runtime closure is not a list")
        canonical_records: dict[str, dict[str, Any]] = {}
        for record in records:
            if type(record) is not dict or set(record) != {"path", "bytes", "raw_sha256"}:
                raise V5BuildError(f"{name} native runtime record drifted")
            key = record["path"].casefold()
            if key in canonical_records:
                raise V5BuildError(f"{name} native runtime universe is ambiguous")
    ...
    if normalized_native["gpu_process_1"] != normalized_native["gpu_process_2"]:
        raise V5BuildError("independent GPU normalized native runtime closure drifted")
    ...
    for candidate in CANDIDATES:
        ...
        if gpu_one_values != gpu_two_values or not cpu_values or len(cpu_values) != len(gpu_one_values):
            raise V5BuildError(f"candidate output geometry/reproducibility drifted: {candidate}")
        maximum = max(abs(float(left) - float(right)) for left, right in zip(cpu_values, gpu_one_values))
        if maximum > 7.5e-5:
            raise V5BuildError(f"CPU/GPU tolerance drifted: {candidate}={maximum}")
        ...
        for worker in workers.values():
            ...
            if (
                ...
                raise V5BuildError(f"candidate causal/deploy boundary drifted: {candidate}")
            for provenance in (
    ...
    return _semantic({
        "schema_version": "expected_pe.causal_valuation_tcn_v5.synthetic_smoke.v1",
...
def _ruff() -> dict[str, Any]:
    if _sha256_file(RUFF_EXECUTABLE) != RUFF_EXECUTABLE_SHA256:
        raise V5BuildError("pinned Ruff executable drifted")
    command = [
    ...
    if result["return_code"] != 0:
        raise V5BuildError("Ruff failed:\n" + result["output_tail"])
    return result
...
def _source_lock_payload() -> dict[str, Any]:
    ...
    if payload.get("authority") != AUTHORITY_ZERO:
        raise V5BuildError("source lock authority drifted")
    return payload
    ...
    *, preflight_process: Mapping[str, Any], preflight: Mapping[str, Any],
    ...
    json_payloads: dict[str, dict[str, Any]] = {
        "ACCESS_RECEIPT.json": _semantic({
            "schema_version": "expected_pe.causal_valuation_tcn_v5.access_receipt.v1",
        ...
        "DESIGN_CONTRACT.json": _semantic({
            "schema_version": "expected_pe.causal_valuation_tcn_v5.design_contract.v1",
            ...
            "candidate_end_to_end_raw_input_receptive_fields": {
                "cvtcn_v5_tcn_residual": 128,
                "cvtcn_v5_grud_residual": 128,
                "cvtcn_v5_static_state_mlp": 1,
            },
        ...
        "ENVIRONMENT_DEPENDENCY_CLOSURE.json": _semantic({
            "schema_version": "expected_pe.causal_valuation_tcn_v5.environment_closure.v1",
        ...
        "INPUT_AGNOSTIC_PREFLIGHT.json": _semantic({
            "schema_version": "expected_pe.causal_valuation_tcn_v5.input_agnostic_preflight.v1",
        ...
        "MANIFEST.json": _semantic({
            "schema_version": "expected_pe.causal_valuation_tcn_v5.manifest.v1",
        ...
        "QUALITY_RECEIPT.json": _semantic({
            "schema_version": "expected_pe.causal_valuation_tcn_v5.quality_receipt.v1",
            "status": "PASS_RUFF_47_TESTS_CPU_TWO_GPU",
        ...
        "SOURCE_CLOSURE.json": _semantic({
            "schema_version": "expected_pe.causal_valuation_tcn_v5.source_closure.v1",
            ...
            "v1_to_v4_immutable_source_and_audit_attested": True,
            "authority": AUTHORITY_ZERO,
        }),
        "STATIC_SOURCE_AUDIT.json": _semantic({
            "schema_version": "expected_pe.causal_valuation_tcn_v5.static_source_audit.v1",
            "status": "PASS_ALL_V4_NO_GO_REPAIRS_AND_V5_ATTACK_REGRESSIONS",
            "p0": 0,
            "p1": 0,
            "p2": 0,
    ...
    if set(output) != set(PAYLOAD_FILES):
        raise V5BuildError("provisional payload universe drifted")
    return output
...
def _write_bundle(bundle: Path, payloads: Mapping[str, bytes]) -> None:
    ...
    seal = _semantic({
        "schema_version": "expected_pe.causal_valuation_tcn_v5.seal.v1",
...
def _verify_bundle(bundle: Path) -> dict[str, Any]:
    ...
    for child in bundle.iterdir():
        ...
        if stat.S_ISLNK(metadata.st_mode) or bool(
            ...
            raise V5BuildError("bundle contains a non-ordinary file member")
        actual.append(child.name)
    if tuple(sorted(actual)) != BUNDLE_FILES:
        raise V5BuildError("bundle exact 14-file universe drifted")
    records = [_file_record(bundle / name, relative_to=bundle) for name in PAYLOAD_FILES]
    expected_checksums = "".join(
        f"{record['raw_sha256']}  {record['path']}\n" for record in records
    ).encode("ascii")
    if (bundle / "CHECKSUMS.sha256").read_bytes() != expected_checksums:
        raise V5BuildError("bundle checksum cross-seal drifted")
    ...
    if semantic != _sha256_bytes(_canonical(seal)):
        raise V5BuildError("bundle seal semantic hash drifted")
    if (
        ...
        raise V5BuildError("bundle seal cross-reference drifted")
    for name in PAYLOAD_FILES:
        ...
        if payload_semantic != _sha256_bytes(_canonical(payload)):
            raise V5BuildError(f"artifact semantic seal drifted: {name}")
        if payload.get("authority", AUTHORITY_ZERO) != AUTHORITY_ZERO:
            raise V5BuildError(f"artifact authority drifted: {name}")
...
def _write_anchor(container: Path) -> None:
    ...
    anchor = _semantic({
        "schema_version": "expected_pe.causal_valuation_tcn_v5.external_anchor.v1",
...
def _verify_container(container: Path) -> dict[str, Any]:
    ...
    for child in container.iterdir():
        ...
        if stat.S_ISLNK(metadata.st_mode) or bool(
            ...
            raise V5BuildError("container contains a reparse/symlink child")
        if stat.S_ISDIR(metadata.st_mode):
            actual[child.name] = "directory"
        elif stat.S_ISREG(metadata.st_mode):
            actual[child.name] = "file"
        else:
            raise V5BuildError("container contains a non-file/non-directory child")
    if tuple(sorted(actual)) != CONTAINER_CHILDREN or actual["bundle"] != "directory" or any(
        ...
        raise V5BuildError("container exact 3-child universe drifted")
    closure = _verify_bundle(container / "bundle")
    ...
    if (container / "EXTERNAL_ANCHOR.sha256").read_bytes() != expected_pointer:
        raise V5BuildError("external anchor pointer drifted")
    ...
    if semantic != _sha256_bytes(_canonical(anchor)):
        raise V5BuildError("external anchor semantic hash drifted")
    if (
        ...
        raise V5BuildError("external anchor cross-seal drifted")
    return {
        **closure,
...
def _safe_remove_provisional(path: Path) -> None:
    ...
    if absolute.parent != outputs or not absolute.name.startswith(
        ".causal_valuation_tcn_v5_provisional."
    ):
        raise V5BuildError("refused unsafe provisional cleanup target")
    if absolute.exists():
...
def main() -> int:
    ...
    if FINAL_OUTPUT.exists() or STAGE.exists():
        raise V5BuildError("final output or fixed verified stage already exists")
    source_lock_sha, runtime_lock_sha = _launcher_hashes()
    provisional = Path(
        tempfile.mkdtemp(prefix=".causal_valuation_tcn_v5_provisional.", dir=OUTPUTS_ROOT)
    )
    try:
        ...
        output = {
            "schema_version": "expected_pe.causal_valuation_tcn_v5.build_result.v1",
