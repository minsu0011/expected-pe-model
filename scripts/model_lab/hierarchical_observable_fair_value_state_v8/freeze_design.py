"""Build and atomically freeze the score-blind H-OFS V8 launcher repair."""
...
def _parse_json_exact(content: bytes, *, label: str) -> dict[str, Any]:
    def reject_duplicates(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, value in pairs:
            if type(key) is not str or key in output:
                raise RuntimeError(f"V8 duplicate or invalid JSON key: {label}:{key}")
            output[key] = value
        return output
    ...
    value = json.loads(
        ...
        parse_constant=lambda token: (_ for _ in ()).throw(
            RuntimeError(f"V8 non-finite JSON token: {label}:{token}")
        ),
    )
    if type(value) is not dict:
        raise RuntimeError(f"V8 JSON root is not an object: {label}")
    return value
...
def _logical_sha256(payload: dict[str, Any]) -> str:
    ...
    if expected != actual:
        raise RuntimeError("V8 artifact logical self-seal drifted")
    return actual
...
def _require_regular(path: Path, *, root: Path) -> bytes:
    if path.is_symlink() or _is_reparse(path) or not path.is_file():
        raise RuntimeError(f"V8 non-regular file: {path}")
    try:
        path.resolve().relative_to(root.resolve())
    except ValueError as error:
        raise RuntimeError(f"V8 file escaped root: {path}") from error
...
def build_preflight_payload(project_root: Path = PROJECT_ROOT) -> dict[str, Any]:
    ...
    return {
        "schema_version": "expected_pe.hofs_v8.dgp_r4_preflight.v1",
        "status": "PASS_V8_ISOLATED_LAUNCHER_REPAIR_PREFLIGHT_NO_MODEL_LAUNCH",
        ...
        "v7_single_run_failure_evidence_sha256": failure_evidence_sha256(),
        "v7_failure_live_receipt": failure,
...
def build_design_bundle_bytes(project_root: Path = PROJECT_ROOT) -> dict[str, bytes]:
    ...
    if dict(source_audit["source_sha256"]).get(launcher_relative) != launcher_sha256:
        raise RuntimeError("V8 launcher is not bound by its source closure")
    from scripts.model_lab.hierarchical_observable_fair_value_state_v8.prediction_launcher import (  # noqa: PLC0415
    ...
    design_lock = sealed_payload(
        {
            "schema_version": "expected_pe.hofs_v8.design_lock.v1",
            "status": "PASS_V8_EXACT_V7_SEMANTICS_LAUNCHER_BOOTSTRAP_REPAIR_LOCK",
            "design_contract_sha256": contract_sha256(),
            "design_contract": contract_payload(),
            "v7_design_binding": copy.deepcopy(V7_DESIGN_BINDING),
            "v7_audit_binding": copy.deepcopy(V7_AUDIT_BINDING),
            ...
            "v7_single_run_failure_evidence_sha256": failure_evidence_sha256(),
            "repair": {
                "r4_canonical_columns_authoritative_module": (
                    "research.model_zoo.hierarchical_observable_fair_value_state_v7."
                    "dgp_r4"
                ),
                "wrong_contracts_import_removed": True,
    ...
    failure_closure = sealed_payload(
        {
            "schema_version": "expected_pe.hofs_v8.v7_failure_closure.v1",
            "status": "PASS_EXACT_SINGLE_V7_ZERO_TASK_ZERO_FIT_FAILURE_BOUND",
            "v7_design_binding": copy.deepcopy(V7_DESIGN_BINDING),
            "v7_audit_binding": copy.deepcopy(V7_AUDIT_BINDING),
            "v7_single_run_failure_evidence": copy.deepcopy(
                V7_SINGLE_RUN_FAILURE_EVIDENCE
            ...
            "v8_repair_scope": "AUTHORITATIVE_IMPORT_AND_SPAWN_BOOTSTRAP_ONLY",
        }
    )
    source_closure = sealed_payload(
        {
            "schema_version": "expected_pe.hofs_v8.source_closure.v1",
            "status": "PASS_EXACT_V8_AND_INHERITED_V7_SOURCE_CLOSURE",
            "source_audit": source_audit,
            "source_file_count": source_audit["source_file_count"],
            "v8_primary_source_file_count": (
                source_audit["v8_primary_source_file_count"]
            ),
    ...
    resource = sealed_payload(
        {
            "schema_version": "expected_pe.hofs_v8.resource_receipt.v1",
    ...
    quality = sealed_payload(
        {
            "schema_version": "expected_pe.hofs_v8.quality_receipt.v1",
            "status": (
                "PASS_V8_67_PLUS_185_RUFF_AND_ACTUAL_32_SPAWN_BOOTSTRAP_PREFLIGHT"
            ),
            "pytest": {
                "python_version": "3.10.19",
                "bytecode_disabled": True,
                "cache_provider_disabled": True,
                "focused_v8_passed": 67,
                "focused_v8_failed": 0,
                "v1_through_v7_cache_free_mirror_passed": 185,
                "v1_through_v7_cache_free_mirror_failed": 0,
            ...
            "actual_prefreeze_worker_bootstrap": {
                "status": (
                    "PASS_EXACT_32_SPAWN_IMPORT_SYMBOL_ORIGIN_HEADER_BOOTSTRAP"
                ),
                "process_start_method": "spawn",
                "worker_count": 32,
                "distinct_worker_pid_count": 32,
                "worker_semantic_receipt_sha256": (
                    "a3b1ee135f4c06c83d1348920620e42cac7b1153e69966824f0ac7c8111b8329"
                ),
                "real_fit_count": 0,
                "real_prediction_count": 0,
    ...
    access = sealed_payload(
        {
            "schema_version": "expected_pe.hofs_v8.access_receipt.v1",
            ...
            "public_header_bootstrap_read_count_per_formal_check": 32,
    ...
    report = (
        "# H-OFS V8 Isolated Launcher Repair Preflight\n\n"
        "Status: PASS_V8_DESIGN_PREFLIGHT_NO_MODEL_LAUNCH\n\n"
        f"Contract: {contract_sha256()}\n\n"
        "V8 preserves the exact V7 estimator, public causal-prefix receipts, "
        ...
        "V7 run ID 20260821T144730 failure with zero tasks, fits, and "
        "predictions and preserves its empty staging directory.\n\n"
        "The launcher now imports R4_CANONICAL_COLUMNS from authoritative "
        "dgp_r4.py. Formal check must spawn exactly 32 Windows workers. Every "
        "initializer imports and attests all execute_task modules and symbols, "
        "their origins and hashes, one metadata-only task, and the exact "
        "public canonical path and header before returning a sealed receipt.\n\n"
        "The pinned Python 3.10 quality run passes 67/67 focused V8 tests and "
        "185/185 cache-free V1-V7 regressions; Ruff 0.12.0 passes. The actual "
        "pre-freeze spawn probe returns 32 sealed receipts from 32 distinct "
        "workers with zero fits and predictions.\n\n"
        ...
        f"independent audit with verdict {V8_AUDIT_VERDICT} is required.\n"
    ).encode("ascii")
    core_payloads = {
    ...
    manifest = sealed_payload(
        {
            "schema_version": "expected_pe.hofs_v8.design_manifest.v1",
            "status": "PASS_V8_DESIGN_READY_FOR_INDEPENDENT_AUDIT",
            "design_contract_sha256": contract_sha256(),
            "v7_design_binding": copy.deepcopy(V7_DESIGN_BINDING),
            "v7_audit_binding": copy.deepcopy(V7_AUDIT_BINDING),
            "v7_single_run_failure_evidence_sha256": failure_evidence_sha256(),
    ...
    seal = sealed_payload(
        {
            "schema_version": "expected_pe.hofs_v8.design_seal.v1",
            "status": "SEALED_V8_PREFLIGHT_STOPPED_FOR_INDEPENDENT_AUDIT",
            "verdict": "GO_NEW_INDEPENDENT_V8_PRELAUNCH_AUDIT_ONLY",
            "design_contract_sha256": contract_sha256(),
            "v7_design_binding": copy.deepcopy(V7_DESIGN_BINDING),
            "v7_audit_binding": copy.deepcopy(V7_AUDIT_BINDING),
            "v7_single_run_failure_evidence_sha256": failure_evidence_sha256(),
    ...
    if tuple(sorted(bundle)) != FINAL_FILE_UNIVERSE:
        raise RuntimeError("V8 design bundle universe drifted")
    return bundle
...
def verify_design_bundle(
    ...
    *,
    ...
    if not _is_sha256(expected_checksums_raw_sha256):
        raise RuntimeError("V8 expected checksum receipt is invalid")
    root = Path(output_root)
    if root.is_symlink() or _is_reparse(root) or not root.is_dir():
        raise RuntimeError("V8 design root is non-regular")
    children = tuple(root.iterdir())
    if any(path.is_symlink() or _is_reparse(path) or not path.is_file() for path in children):
        raise RuntimeError("V8 design contains a non-regular child")
    ...
    if names != FINAL_FILE_UNIVERSE or len({name.casefold() for name in names}) != len(names):
        raise RuntimeError(f"V8 design universe drifted: {names!r}")
    ...
    if _sha256(contents["CHECKSUMS.sha256"]) != expected_checksums_raw_sha256:
        raise RuntimeError("V8 external checksum receipt drifted")
    ledger_names = tuple(
    ...
    if len(lines) != len(ledger_names):
        raise RuntimeError("V8 checksum ledger count drifted")
    for position, line in enumerate(lines):
        ...
        if (
            ...
            raise RuntimeError(f"V8 checksum ledger drifted: {position}")
    parsed = {
    ...
    if (
        ...
        or manifest.get("v7_single_run_failure_evidence_sha256")
        != failure_evidence_sha256()
        or seal.get("v7_single_run_failure_evidence_sha256")
        != failure_evidence_sha256()
        ...
        raise RuntimeError("V8 manifest or seal cross-binding drifted")
    expected_core = tuple(
        name
        for name in FINAL_FILE_UNIVERSE
    ...
    if tuple(sorted(manifest.get("core_files", {}))) != expected_core:
        raise RuntimeError("V8 manifest core universe drifted")
    for name in expected_core:
        ...
        if (
            ...
            raise RuntimeError(f"V8 manifest core receipt drifted: {name}")
        if name.endswith(".json") and receipt.get("logical_sha256") != logical[name]:
            raise RuntimeError(f"V8 manifest logical receipt drifted: {name}")
    sealed_names = tuple(
        name
        for name in FINAL_FILE_UNIVERSE
...
def freeze_design_bundle(output_root: Path = DEFAULT_OUTPUT_ROOT) -> dict[str, Any]:
    ...
    if raw_target.is_symlink() or _is_reparse(raw_target):
        raise RuntimeError("V8 freeze target reparse point is forbidden")
    target = raw_target.resolve()
    outputs_root = (PROJECT_ROOT / "outputs").resolve()
    if target.parent != outputs_root or target != DEFAULT_OUTPUT_ROOT.resolve():
        raise RuntimeError("V8 freeze target must be the exact declared outputs child")
    staging = outputs_root / f".{target.name}.staging"
    if target.exists() or staging.exists():
        raise FileExistsError("V8 freeze target or staging already exists")
    ...
    if staging.exists() or not target.is_dir():
        raise RuntimeError("V8 atomic freeze publication failed")
        ...
        json.dumps(
            {
                ...
                "failure_evidence_sha256": (
                    payload["v7_single_run_failure_evidence_sha256"]
                ),
