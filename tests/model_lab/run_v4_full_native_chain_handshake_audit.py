"""Audit shim V2 -> managed V2 -> V4 custody without consuming identity."""
...
SHIM = PROJECT_ROOT / (
    "build/r8r8_native_environment_shim_v2_immutable_20260823/"
    "ExpectedPeNativeEnvironmentShimV2.exe"
)
EXTERNAL_AUDIT = PROJECT_ROOT / "tests/model_lab/run_v4_external_native_handshake_audit.py"
GOVERNED = (
...
def main() -> int:
    ...
    if before != (False, False, False):
        raise RuntimeError("V4 governed identity existed before full-chain audit")
    hostile_environment = {
        **os.environ,
    ...
    process = subprocess.Popen(
        (str(SHIM), "--audit-external-v4-handshake-window-no-python-no-identity"),
        cwd=PROJECT_ROOT,
    ...
    try:
        ...
        if ready.get("status") != "READY_EXTERNAL_V4_HANDSHAKE_WINDOW_NO_PYTHON_NO_IDENTITY":
    ...
    if (
        ...
        != "PASS_FULL_V4_EXTERNAL_NATIVE_HANDSHAKE_NO_EXECUTION_NO_IDENTITY"
        or final.get("status") != "PASS_EXTERNAL_V4_HANDSHAKE_WINDOW_NO_PYTHON_NO_IDENTITY"
        ...
        json.dumps(
            {
                ...
                "external_v4_receipt": external_receipt,
                ...
                "status": "PASS_FULL_NATIVE_SHIM_V2_MANAGED_V2_V4_HANDSHAKE_NO_EXECUTION",
            },
            sort_keys=True,
            separators=(",", ":"),
        )
