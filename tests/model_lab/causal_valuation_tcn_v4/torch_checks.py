"""Synthetic-only V3 finding reproductions and V4 direct-surface attacks."""
...
from research.model_zoo.causal_valuation_tcn_v4.contracts import (
    CANDIDATE_IDS,
    EMBARGO_SESSIONS,
    FEATURE_COLUMNS,
...
from research.model_zoo.causal_valuation_tcn_v4.custody import (
    CausalSequenceBatch,
    FoldRowCustody,
...
from research.model_zoo.causal_valuation_tcn_v4.models import (
    TorchSequenceCustody,
    build_fixed_model,
    torch_sequence_custody_sha256,
)
from research.model_zoo.causal_valuation_tcn_v4.path_guard import (
    CausalPathError,
    require_clean_tree,
)
from research.model_zoo.causal_valuation_tcn_v4.smoke_worker import _fixture
from research.model_zoo.causal_valuation_tcn_v4.source_audit import run_source_audit
from research.model_zoo.causal_valuation_tcn_v4.training import (
...
STATIC_ID = "cvtcn_v4_static_state_mlp"
...
def _small_source(*, gap: int = 10):
    ...
    return build_canonical_source_custody(
        external_source_sha256=hashlib.sha256(b"small-v4-source").hexdigest(),
        entity_ids=("E", "E"),
...
class V4ContractChecks(unittest.TestCase):
    @classmethod
    ...
    def test_15_candidate_membership_checked_by_model(self) -> None:
        with self.assertRaises(CausalValuationContractError):
            build_fixed_model("cvtcn_v4_tcn_residual")(self.train_inputs)
    ...
    def test_17_sparse_validation_history_overlap_is_rejected(self) -> None:
        ...
        rows.sort()
        source = build_canonical_source_custody(
            external_source_sha256=hashlib.sha256(b"sparse-v4").hexdigest(),
    ...
    def test_23_resealed_caller_prediction_values_are_rejected(self) -> None:
        ...
        value_hash = _tensor_hash(
            values, domain=b"expected_pe.causal_valuation_tcn_v4.predictions.v2\0"
        )
        forged = _clone(
    ...
    def test_25_unrelated_state_artifact_cannot_replace_bound_artifact(self) -> None:
        ...
        state = build_fixed_model(STATIC_ID).deployable_state_dict()
    ...
    def test_34_exact_clean_tree_rejects_extra_pyc_and_reparse(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            with mock.patch(
                "research.model_zoo.causal_valuation_tcn_v4.path_guard._metadata_is_reparse",
                return_value=True,
            ):
                with self.assertRaises(CausalPathError):
                    require_clean_tree(
    ...
    def test_36_launcher_and_candidate_universe_are_exact(self) -> None:
        launcher = Path(
            "scripts/model_lab/causal_valuation_tcn_v4/trusted_launcher.py"
    ...
    def test_37_v1_to_v3_and_live_v4_source_custody_passes(self) -> None:
...
if __name__ == "__main__":
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(V4ContractChecks)
