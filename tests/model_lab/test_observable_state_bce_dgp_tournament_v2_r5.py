"""Score-free source/contract attacks for the isolated V2 R5 repair."""
...
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r5 import authority
...
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r5.design import (
    CANDIDATE_CONTRACT,
...
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r5.executable import (
    executable_manifest_payload,
)
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r5.identity import (
    process_token_identity,
)
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r5.lineage import (
    R4_NO_GO_AUDIT_SEMANTIC,
    R4_NO_GO_PROBE_RESULT_SEMANTIC,
    r4_supersession_payload,
)
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r5.path_guard import (
    R5PathError,
    require_clean_tree,
    require_lexically_exact_path,
)
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r5.signature import (
    R5SignatureError,
...
PACKAGE = ROOT / "research/model_zoo/observable_state_bce_dgp_tournament_v2_r5"
BOOTSTRAP = ROOT / (
    "scripts/model_lab/observable_state_bce_dgp_tournament_v2_r5/"
    "trusted_bootstrap.py"
)
pytestmark = pytest.mark.skipif(sys.platform != "win32", reason="R5 is Windows-fixed")
...
def _valid_audit(context: dict[str, object]) -> dict[str, object]:
    return seal_payload(
        {
            "schema_version": authority.R5_AUDIT_SCHEMA,
            "audit_revision": "R5",
            "generated_at_utc": "2026-08-21T00:00:00+00:00",
            "status": authority.R5_AUDIT_STATUS,
            "verdict": authority.R5_AUDIT_VERDICT,
...
def _valid_approval(bindings: dict[str, object]) -> dict[str, object]:
    return seal_payload(
        {
            "schema_version": authority.R5_APPROVAL_SCHEMA,
            "status": authority.R5_APPROVAL_STATUS,
            "action": authority.R5_APPROVAL_ACTION,
            "decision": authority.R5_APPROVAL_DECISION,
...
def test_bootstrap_rejects_pyc_pyd_dll_and_extra_file(tmp_path: Path) -> None:
    ...
    for name in ("evil.py", "evil.pyc", "evil.pyd", "evil.dll", "evil.zip"):
        ...
        with pytest.raises(R5PathError):
            require_clean_tree(
...
def test_bootstrap_binds_all_initializers_origins_hashes_loaders() -> None:
    ...
    assert "research.model_zoo.observable_state_bce_dgp_tournament_v2_r5" in modules
    assert manifest["governed_loader_required"] == "__main__.BoundSourceLoader"
    assert set(path.name for path in PACKAGE.iterdir()) == set(R5_SOURCE_FILENAMES)
...
def test_bootstrap_verified_bytes_import_attestation_executes_before_action() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="r5_unit_source_only_") as temporary:
        stage = Path(temporary)
        trusted["_stage"](manifest, manifest_raw, stage)
        parent = os.environ
        environment = {
            "PYTHONDONTWRITEBYTECODE": "1",
            "R5_BOOTSTRAP_COMMAND": "attest_import_only",
            "R5_BOOTSTRAP_PARENT_NONCE": "a" * 64,
            "R5_EXECUTABLE_MANIFEST_RAW_SHA256": __import__("hashlib")
            .sha256(manifest_raw)
            .hexdigest(),
            "R5_STAGE_ROOT": str(stage),
...
def test_R4_NO_GO_full_bundle_supersession_exact() -> None:
    payload = r4_supersession_payload()
    assert payload["R4_NO_GO"]["audit_semantic_sha256"] == R4_NO_GO_AUDIT_SEMANTIC
    assert (
        payload["R4_NO_GO"]["probe_result_semantic_sha256"]
        == R4_NO_GO_PROBE_RESULT_SEMANTIC
    )
    assert payload["R4_NO_GO"]["severity_counts"] == {"P0": 0, "P1": 1, "P2": 0}
...
def test_transaction_lock_is_process_lifetime_OS_lock() -> None:
    source = inspect.getsource(
        __import__(
            "research.model_zoo.observable_state_bce_dgp_tournament_v2_r5.transaction",
    ...
    with pytest.raises(R5TransactionError):
        _execute_authenticated_reservation(
            "RESERVE_OBSERVABLE_STATE_BCE_DGP_TOURNAMENT_V2_R5_TRANSACTIONALLY_ONCE",
            _BOUND_BOOTSTRAP_CAPABILITY,
        )
...
def test_named_test_results_and_raw_logs_bound() -> None:
    assert len(REQUIRED_NAMED_PROBES) == 24
    assert len(set(REQUIRED_NAMED_PROBES)) == 24
...
def test_audit_minimal_unbound_extra_missing_wrong_hash_resealed_attacks_reject(
    ...
    with pytest.raises(authority.R5ContractError):
...
def test_minimal_unbound_extra_missing_wrong_hash_audit_approval_rejected(
    ...
    with pytest.raises(R5SignatureError):
