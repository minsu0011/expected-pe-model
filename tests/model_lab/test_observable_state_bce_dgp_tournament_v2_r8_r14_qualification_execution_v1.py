...
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r8_r14_qualification_execution_v1.authority import (
    DGPS,
    QUALIFICATION_SEEDS,
    MemoryQualificationAuthority,
    QualificationAuthorityError,
)
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r8_r14_qualification_execution_v1.protected_role import (
    _logical_frame_sha256 as protected_pre_serialization_digest,
)
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r8_r14_qualification_execution_v1.protected_role import (
    _public_round_trip_logical_sha256,
)
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r8_r14_qualification_execution_v1.public_role import (
...
PRELAUNCH_R8_R12 = PROJECT_ROOT / (
    "outputs/model_zoo_r8_r12_qualification_execution_v1_prelaunch_audit_go_20260823"
)
TERMINAL_R8_R13 = PROJECT_ROOT / (
    "research/model_zoo/observable_state_bce_dgp_tournament_v2_r8_r13_"
    "qualification_execution_v1"
)
...
def test_r8_r12_prelaunch_evidence_is_preserved_without_activation() -> None:
    assert hashlib.sha256((PRELAUNCH_R8_R12 / "PRELAUNCH_AUDIT.json").read_bytes()).hexdigest() == (
        "a66a26d6f39d27f03351d1c851d537eb0a7f337daff66571b1b9b64c55eb91d1"
    )
    assert not (
        PROJECT_ROOT
        / "outputs/model_zoo_observable_state_bce_dgp_tournament_v2_r8_r12_"
        "qualification_activation_20260823"
    ).exists()


def test_terminal_r8_r13_finalizer_failure_evidence_is_preserved() -> None:
    failure = (
        PROJECT_ROOT
    ...
    assert hashlib.sha256(failure.read_bytes()).hexdigest() == (
        "7bcd734ab67c7f9d8543dd3e9258875992f846492d58327c94eeb45ac2c1ae5b"
    )
    assert "canonical_json_bytes(metadata).casefold()" in (
        TERMINAL_R8_R13 / "controller.py"
...
def test_public_successor_has_no_live_deny_and_no_protected_import() -> None:
    ...
    assert 'canonical["ml_expected_pe"]' in public
    assert 'overlay["v04_expected_pe"]' in public
    assert '"row_position": np.arange(len(canonical), dtype=np.int64)' in public
    assert '"observed_pe":' not in public
    assert 'endswith("__pe")' not in public
...
def test_role_worker_exposes_only_fixed_closed_commands() -> None:
    ...
    assert "r8_r14_qualification_execution_v1.protected_role" in source
    assert "r7_qualification_generation.protected_role" not in source


def test_r8_r14_uses_new_one_shot_paths_and_explicit_digest_contract() -> None:
    ...
    assert "r8_r14_qualification_activation_20260823" in custodian
    assert "qualification_activation_20260822" not in custodian
    assert 'PUBLIC_RUN_ID = "20260824T000005"' in runner
    ...
    assert "canonical_json_bytes(metadata).lower()" in (
        PACKAGE / "controller.py"
    ).read_text(encoding="utf-8")
    assert "FULL_IDENTITIES_HEADER_CONTENT_SHA256" in (
        PACKAGE / "controller.py"
    ).read_text(encoding="utf-8")
    assert (
        "eef0415e9d126f903e493f225b405569b28a8f29432b6be513e0237e4604848c"
        in (PACKAGE / "contracts.py").read_text(encoding="utf-8")
    )


def test_r8_r14_binds_real_replay_authority_and_keeps_venv_dependencies() -> None:
    design = PROJECT_ROOT / (
    ...
    assert 'PROJECT_ROOT.parent / "pe-regime-v03-source.zip"' in worker
    assert "if resolved in {comparator_archive, pinned_python}" in worker
    assert "return not write" in worker
    public_run = worker.split('if command == "role-public-run":', 1)[1].split(
        'if command == "role-public-finalize":', 1
    )[0]
    assert public_run.index("inventory, runtime = _design()") < public_run.index(
        "_public_guard(root)"
    )
