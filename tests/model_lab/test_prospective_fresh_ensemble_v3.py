...
from research.model_zoo.prospective_fresh_ensemble_v3.custody import (
    VerifiedCapability,
    assert_case_pair,
...
from research.model_zoo.prospective_fresh_ensemble_v3.evaluation import (
    _verify_locked_evaluation_surface,
    write_heldout_report_and_terminal,
    write_qualification_report_and_terminal,
)
from research.model_zoo.prospective_fresh_ensemble_v3.generation import run_generation
from research.model_zoo.prospective_fresh_ensemble_v3.heldout_authority import (
    _verify_passing_qualification,
)
from research.model_zoo.prospective_fresh_ensemble_v3.pid_handshake import VerifiedReadyClaim
from research.model_zoo.prospective_fresh_ensemble_v3.prediction import run_predictions
from research.model_zoo.prospective_fresh_ensemble_v3.process_runner import (
    initialize_actual_child_worker,
)
from research.model_zoo.prospective_fresh_ensemble_v3.precommit import (
    build_design_lock,
    build_preflight,
    build_qualification_descriptor,
...
def test_alternate_prediction_path_is_rejected(
    ...
    monkeypatch.setattr(
        "research.model_zoo.prospective_fresh_ensemble_v3.custody.inspect_canonical_identity",
...
def test_direct_child_rechecks_authority_before_ready_claim(
    ...
    monkeypatch.setattr(
        "research.model_zoo.prospective_fresh_ensemble_v3.process_runner.verify_stage_descriptor",
        lambda *_args, **_kwargs: descriptor,
    )

    def reject_authority(_root: Path) -> dict[str, Any]:
        raise ProspectiveContractError("missing V3 approval")

    monkeypatch.setattr(
        "research.model_zoo.prospective_fresh_ensemble_v3.qualification_authority.require_qualification_parent_authority",
        reject_authority,
    )
    monkeypatch.setattr(
        "research.model_zoo.prospective_fresh_ensemble_v3.process_runner.publish_child_ready_claim",
        lambda **_kwargs: pytest.fail("ready claim published before child authority check"),
    )
    with pytest.raises(ProspectiveContractError, match="missing V3 approval"):
        initialize_actual_child_worker(
...
def test_qualification_modules_have_no_heldout_resolver() -> None:
    paths = (
        PROJECT_ROOT
        / "research/model_zoo/prospective_fresh_ensemble_v3/qualification_authority.py",
        PROJECT_ROOT
        / "research/model_zoo/prospective_fresh_ensemble_v3/qualification_process.py",
        PROJECT_ROOT
        / "scripts/model_lab/prospective_fresh_ensemble_v3/run_qualification_stage.py",
        PROJECT_ROOT
        / "scripts/model_lab/prospective_fresh_ensemble_v3/evaluate_qualification.py",
    )
...
def test_v3_stage_clis_do_not_accept_caller_artifact_paths() -> None:
    paths = (
        PROJECT_ROOT / "scripts/model_lab/prospective_fresh_ensemble_v3/run_qualification_stage.py",
        PROJECT_ROOT / "scripts/model_lab/prospective_fresh_ensemble_v3/evaluate_qualification.py",
        PROJECT_ROOT / "scripts/model_lab/prospective_fresh_ensemble_v3/run_heldout_stage.py",
        PROJECT_ROOT / "scripts/model_lab/prospective_fresh_ensemble_v3/evaluate_heldout.py",
    )
    for path in paths:
...
def test_score_free_precommit_components_close_before_any_reservation(
    tmp_path: Path,
) -> None:
    output = (tmp_path / "v3_dry_precommit").resolve()
    output.mkdir()
    closure = build_source_dependency_closure(PROJECT_ROOT)
    closure_path = output / "SOURCE_DEPENDENCY_CLOSURE.json"
    write_json_exclusive(closure_path, closure)
    overrides_path = output / "V04_OVERRIDES.json"
    write_json_exclusive(overrides_path, V04_OVERRIDES)
    reservation = build_reservation_binding(PROJECT_ROOT)
    reservation_path = output / "RESERVATION_BINDING.json"
    write_json_exclusive(reservation_path, reservation)
    supersession = build_supersession(PROJECT_ROOT)
    supersession_path = output / "SUPERSESSION.json"
    write_json_exclusive(supersession_path, supersession)
    design = build_design_lock(
        PROJECT_ROOT,
        closure,
        closure_path,
        overrides_path,
    ...
    descriptor = build_qualification_descriptor(
        output,
        PROJECT_ROOT,
        closure,
        closure_path,
    ...
    descriptor_path = output / "QUALIFICATION_STAGE_DESCRIPTOR.json"
    ...
    assert preflight["status"] == "GO_V3_PRECOMMIT_ONLY_LAUNCH_BLOCKED"
    assert preflight["new_seed_reservation_created"] is False
    assert preflight["data_generated"] is False
    assert preflight["fit_calls"] == 0
    assert preflight["scores_computed"] is False
    assert not (output / "SEED_RESERVATION.json").exists()
