"""Exercise recovery V4 under an already active native DACL window.

This audit never calls ``main``, creates the V4 supervisor claim, creates the
...
EXPECTED_SHA256 = "5266f4864976a11cb1d2c940657e3c87e1824e2b234a229ffea2bbecb86b468c"
EXPECTED_SIZE = 100356
CLAIM = PROJECT_ROOT / "build/pc_r8r8_preimport_source_supervisor_v4_recovery_once_20260823"
...
def main() -> int:
    ...
    if before != (False, False, False):
        raise RuntimeError("V4 one-shot identity or output existed before handshake audit")
    raw = SUPERVISOR.read_bytes()
    if len(raw) != EXPECTED_SIZE or hashlib.sha256(raw).hexdigest() != EXPECTED_SHA256:
        raise RuntimeError("V4 supervisor bytes drifted before handshake audit")
    namespace: dict[str, Any] = {
        "__builtins__": __builtins__,
        "__file__": str(SUPERVISOR),
        "__name__": "r8r8_v4_external_native_handshake_audit",
    }
    ...
    try:
        ...
        if (
            ...
            raise RuntimeError("external V4 custody universe drifted")
        ...
        if any(
            ...
            raise RuntimeError("external V4 DACL restoration was not delegated")
        after = tuple(path.exists() for path in governed)
        if after != before:
            raise RuntimeError("handshake audit consumed a V4 one-shot identity or output")
        print(
            json.dumps(
                {
                    ...
                    "status": "PASS_FULL_V4_EXTERNAL_NATIVE_HANDSHAKE_NO_EXECUTION_NO_IDENTITY",
                },
                sort_keys=True,
                separators=(",", ":"),
            )
