"""Synthetic-only V3 finding reproductions and V6 direct-surface attacks."""
...
from research.model_zoo.causal_valuation_tcn_v6.contracts import (
    CANDIDATE_IDS,
...
from research.model_zoo.causal_valuation_tcn_v6.custody import (
    CausalSequenceBatch,
    FoldRowCustody,
...
from research.model_zoo.causal_valuation_tcn_v6.models import (
...
from research.model_zoo.causal_valuation_tcn_v6.path_guard import (
    CausalPathError,
    require_clean_tree,
)
from research.model_zoo.causal_valuation_tcn_v6.smoke_worker import _fixture
from research.model_zoo.causal_valuation_tcn_v6.source_audit import run_source_audit
from research.model_zoo.causal_valuation_tcn_v6.training import (
...
def _small_source(*, gap: int = 10):
    ...
    return build_canonical_source_custody(
        external_source_sha256=hashlib.sha256(b"small-v6-source").hexdigest(),
        entity_ids=("E", "E"),
...
def _receptive_field_fixture():
    ...
    source = build_canonical_source_custody(
        external_source_sha256=hashlib.sha256(b"v6-rf-boundary-fixture").hexdigest(),
    ...
    *, training_inputs: TorchSequenceCustody, validation: TorchSequenceCustody
    ...
    forged = _clone(
        ...
        center_values_sha256=_tensor_hash(
            future_center,
            domain=b"expected_pe.causal_valuation_tcn_v6.training_centers.v1\0",
        ),
        custody_sha256="0" * 64,
    )
    object.__setattr__(
...
class V6ContractChecks(unittest.TestCase):
    @classmethod
    ...
    def test_15_candidate_membership_checked_by_model(self) -> None:
        with self.assertRaises(CausalValuationContractError):
            build_fixed_model(
                "cvtcn_v6_tcn_residual", training_inputs=self.train_inputs
            )
    ...
    def test_17_sparse_validation_history_overlap_is_rejected(self) -> None:
        ...
        rows.sort()
        source = build_canonical_source_custody(
            external_source_sha256=hashlib.sha256(b"sparse-v6").hexdigest(),
    ...
    def test_23_resealed_caller_prediction_values_are_rejected(self) -> None:
        ...
        value_hash = _tensor_hash(
            values, domain=b"expected_pe.causal_valuation_tcn_v6.predictions.v2\0"
        )
        forged = _clone(
    ...
    def test_34_exact_clean_tree_rejects_extra_pyc_and_reparse(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            with mock.patch(
                "research.model_zoo.causal_valuation_tcn_v6.path_guard._metadata_is_reparse",
                return_value=True,
            ):
                with self.assertRaises(CausalPathError):
                    require_clean_tree(
    ...
    def test_36_launcher_and_candidate_universe_are_exact(self) -> None:
        launcher = Path(
            "scripts/model_lab/causal_valuation_tcn_v6/trusted_launcher.py"
    ...
    def test_37_v1_to_v5_and_live_v6_source_custody_passes(self) -> None:
    ...
    def test_54_copy_deepcopy_and_attribute_substitution_fail_before_forward(self) -> None:
        validation = TorchSequenceCustody.from_canonical_fold(
            self.batch,
            self.fold,
            candidate_id=STATIC_ID,
        ...
        original = build_fixed_model(
            STATIC_ID, training_inputs=self.train_inputs
        ).eval()
        shallow = copy.copy(original)
        shallow.encoder = copy.deepcopy(original.encoder)
        deep = copy.deepcopy(original)
        substituted = build_fixed_model(
            STATIC_ID, training_inputs=self.train_inputs
        ).eval()
        for attacked in (shallow, deep, substituted):
            with torch.no_grad():
                attacked.encoder.training_center.copy_(future_center)
            object.__setattr__(attacked, "_training_center_custody", forged)
            with self.assertRaises(CausalValuationContractError):
                attacked(validation, training_inputs=self.train_inputs)
...
if __name__ == "__main__":
    suite = unittest.defaultTestLoader.loadTestsFromTestCase(V6ContractChecks)
