...
from scripts.model_lab.hierarchical_observable_fair_value_state_v7 import (
    freeze_design as freeze_design_v7,
    prediction_launcher as prediction_launcher_v7,
)
...
def _valid_observed_for_fit(source: pd.DataFrame) -> pd.Series:
    observed = source["observed_pe"].copy()
    observed.iloc[1:3] = np.nan
    return observed
...
def fitted_case():
    ...
    capability = fit_chronological_prefix_v7(
        ...
        observed_pe=_valid_observed_for_fit(source),
...
@pytest.fixture(scope="module")
def public_closure():
    return build_r4_input_closure_v7(PROJECT_ROOT)
...
def test_contract_lineage_geometry_runtime_and_new_identity() -> None:
    ...
    assert payload["candidate"]["candidate_id"] == ("hofs_v7_huber_soft_pool_state_dgp_r4_blocked")
    assert payload["v6_independent_go_audit_binding"] == V6_INDEPENDENT_AUDIT_BINDING
    assert payload["v6_design_freeze_binding"] == V6_DESIGN_FREEZE_BINDING
    assert payload["v6_zero_fit_failure_evidence"] == V6_ZERO_FIT_FAILURE_EVIDENCE
    assert payload["r4_public_input_binding"] == R4_INPUT_BINDING
    assert V6_INDEPENDENT_AUDIT_BINDING["severity_counts"] == {
        "P0": 0,
        "P1": 0,
        "P2": 0,
    }
    fixed = payload["fixed_hyperparameters"]
    assert fixed["allowed_causal_invalid_positions_per_entity"] == [0, 1, 2]
    assert fixed["minimum_warm_prefix_rows_per_entity"] == 501
...
def test_public_r4_closure_is_exact_and_score_blind(public_closure) -> None:
    ...
    assert len(public_closure["task_causal_prefix_receipts"]) == 50
    assert all(
        item["per_entity"][0]["causal_invalid_positions"] == [0, 1, 2]
        and item["per_entity"][0]["observed_pe_nonfinite_positions"] == [0, 1]
        and item["per_entity"][0]["causal_offset_nonfinite_positions"] == [0, 1, 2]
        and item["per_entity"][0]["first_prefix_requested_row_count"] == 504
        and item["per_entity"][0]["first_prefix_nonwarm_row_count"] == 3
        and item["per_entity"][0]["first_prefix_warm_row_count"] == 501
        and item["per_entity"][0]["later_unexpected_invalid_row_count"] == 0
        and item["per_entity"][0]["decision_invalid_row_count"] == 0
        and item["per_entity"][0]["estimator_sufficiency_passed"] is True
        for item in public_closure["task_causal_prefix_receipts"]
    )
...
def test_public_canonical_adapter_is_schema_restricting() -> None:
    ...
    adapted = adapt_r4_canonical_source_v7(canonical)
...
def test_public_causal_prefix_extra_moved_and_noncontiguous_attacks_fail_closed() -> None:
    path = R4_ROOT / "replays/pass_1/seed_2026082001/dgp_A/canonical150.csv"
    canonical = pd.read_csv(path)
    extra = canonical.copy()
    extra.loc[100, "observed_pe"] = np.nan
    with pytest.raises(HierarchicalStateV7ContractError, match="causal invalid-prefix"):
        _causal_invalid_prefix_receipt(extra)
    moved = canonical.copy()
    moved.loc[1, "observed_pe"] = canonical.loc[2, "observed_pe"]
    moved.loc[10, "observed_pe"] = np.nan
    with pytest.raises(HierarchicalStateV7ContractError, match="causal invalid-prefix"):
        _causal_invalid_prefix_receipt(moved)
    insufficient = canonical.copy()
    insufficient.loc[503, "observed_pe"] = np.nan
    with pytest.raises(HierarchicalStateV7ContractError, match="causal invalid-prefix"):
        _causal_invalid_prefix_receipt(insufficient)
...
def test_expected_regime_warmup_above_199_fails_closed() -> None:
    ...
    with pytest.raises(HierarchicalStateV7ContractError, match="sealed maximum"):
        build_hierarchical_state_features_v7(source)
...
def test_expected_regime_warmup_is_forbidden_in_decision_block() -> None:
    ...
    with pytest.raises(HierarchicalStateV7ContractError, match="forbidden in a decision block"):
        build_decision_batch_v7(state, requested)
...
def test_unexpected_decision_malformed_gate_uses_exact_block_denominator() -> None:
    ...
    state = build_hierarchical_state_features_v7(source)
    requested = _block_identities(source)
    with pytest.raises(HierarchicalStateV7ContractError, match="fallback limit"):
        build_decision_batch_v7(state, requested)


def test_first_fold_receipts_exact_504_equals_3_plus_501(fitted_case) -> None:
    _, _, capability = fitted_case
    receipt = capability.fit.fit_receipt
    assert receipt.prefix_entity_row_counts == (("ENTITY_A", 504, 3, 501),)
    assert receipt.prefix_entity_causal_invalid_positions == (("ENTITY_A", (0, 1, 2)),)
    assert len(receipt.prefix_entity_causal_invalid_positions_sha256) == 64
    assert receipt.requested_row_count == 504
    assert receipt.causal_prefix_nonwarm_row_count == 3
    assert receipt.dropped_nonwarm_row_count == 3
    assert receipt.fit_row_count == 501
    assert receipt.final_kkt_violation <= 1e-10
    assert receipt.expected_regime_warmup_prefix_count == 0


def test_prefix_and_extra_moved_noncontiguous_invalid_attacks_fail_before_solver() -> None:
    short = _source(rows=524)
    requested = _block_identities(short, start=503)
    with pytest.raises(HierarchicalStateV7ContractError, match="prefix"):
        fit_chronological_prefix_v7(
            short,
            decision_block_identities=requested,
            observed_pe=_valid_observed_for_fit(short),
            research_dgp_groups=short["research_dgp_id"],
        )
    source = _source()
    requested = _block_identities(source)
    observed = _valid_observed_for_fit(source)
    observed.iloc[100] = np.nan
    with pytest.raises(HierarchicalStateV7ContractError, match="invalid-position"):
        fit_chronological_prefix_v7(
            source,
            decision_block_identities=requested,
    ...
    moved = _valid_observed_for_fit(source)
    moved.iloc[2] = source["observed_pe"].iloc[2]
    moved.iloc[3] = np.nan
    with pytest.raises(HierarchicalStateV7ContractError, match="invalid-position"):
        fit_chronological_prefix_v7(
            source,
    ...
    missing_allowed = _valid_observed_for_fit(source)
    missing_allowed.iloc[2] = source["observed_pe"].iloc[2]
    with pytest.raises(HierarchicalStateV7ContractError, match="invalid-position"):
        fit_chronological_prefix_v7(
            source,
            decision_block_identities=requested,
            observed_pe=missing_allowed,
            research_dgp_groups=source["research_dgp_id"],
        )
...
def test_valid_block_uses_one_parameter_hash_and_zero_updates(fitted_case) -> None:
    ...
    assert output.decision_block_set_membership_sha256 == decision_block_set_membership_sha256_v7(
        requested
    )
...
def test_reverse_order_is_rejected_without_silent_sort_and_set_hash_is_stable() -> None:
    ...
    assert decision_block_set_membership_sha256_v7(reversed_request) == (
        decision_block_set_membership_sha256_v7(requested)
    )
    with pytest.raises(HierarchicalStateV7ContractError, match="not already in exact canonical"):
        decision_block_ordered_membership_sha256_v7(reversed_request)
    with pytest.raises(HierarchicalStateV7ContractError, match="not already in exact canonical"):
        build_decision_batch_v7(state, reversed_request)
...
def test_arbitrary_permutation_and_duplicate_identity_are_rejected() -> None:
    ...
    with pytest.raises(HierarchicalStateV7ContractError, match="not already in exact canonical"):
        build_decision_batch_v7(state, permuted)

    duplicated = requested.copy()
    duplicated.iloc[6] = duplicated.iloc[5]
    with pytest.raises(HierarchicalStateV7ContractError, match="one-to-one"):
        build_decision_batch_v7(state, duplicated)
...
def test_canonical_date_cross_block_substitution_is_rejected_by_exact_coverage() -> None:
    ...
    with pytest.raises(HierarchicalStateV7ContractError, match="does not exactly cover"):
        build_decision_batch_v7(state, crossed)
...
def test_canonical_endpoint_rows_and_source_positions_are_exactly_bound() -> None:
    ...
    assert batch.decision_source_positions_sha256 == decision_source_positions_sha256_v7(
        requested,
    ...
    with pytest.raises(HierarchicalStateV7ContractError, match="strictly increasing"):
        decision_source_positions_sha256_v7(
            requested,
            source_state_sha256=batch.source_state_sha256,
...
def test_fit_parameter_batch_and_output_manifest_share_all_order_custody(fitted_case) -> None:
    ...
    ordered = decision_block_ordered_membership_sha256_v7(requested)
    set_membership = decision_block_set_membership_sha256_v7(requested)
...
def test_removed_shifted_and_single_date_block_attacks_fail(fitted_case) -> None:
    source, requested, capability = fitted_case
    with pytest.raises(HierarchicalStateV7ContractError):
        run_frozen_decision_block_v7(
            source,
    ...
    with pytest.raises(HierarchicalStateV7ContractError, match="multiple"):
        build_decision_batch_v7(build_hierarchical_state_features_v7(source), single)


def test_update_and_membership_parameter_constructor_attacks_fail(fitted_case) -> None:
    _, _, capability = fitted_case
    parameters = capability.fit.parameters
    with pytest.raises(HierarchicalStateV7ContractError, match="block receipt"):
...
def test_parameter_artifact_round_trip_and_fully_resealed_membership_attack(
    ...
    with pytest.raises(HierarchicalStateV7ContractError, match="custody drifted"):
        load_frozen_parameter_bundle(
            attack_root,
...
def test_fully_resealed_causal_invalid_position_receipt_attack_fails(
    tmp_path: Path,
    fitted_case,
) -> None:
    _, _, capability = fitted_case
    attacked = dict(build_frozen_parameter_bundle_bytes(capability.fit))
    fit_receipt = json.loads(attacked["FIT_RECEIPT.json"].decode("ascii"))
    fit_receipt["prefix_entity_causal_invalid_positions"][0][1] = [0, 1, 3]
    fit_receipt["prefix_entity_causal_invalid_positions_sha256"] = hashlib.sha256(
        canonical_json_bytes([["ENTITY_A", [0, 1, 3]]])
    ).hexdigest()
    attacked["FIT_RECEIPT.json"] = _json_bytes(fit_receipt)
    manifest = json.loads(attacked["MANIFEST.json"].decode("ascii"))
    manifest["fit_receipt_sha256"] = hashlib.sha256(
        canonical_json_bytes(fit_receipt)
    ).hexdigest()
    ...
    attack_root = tmp_path / "invalid-position-attack"
    attack_checksum = _write_payloads(attack_root, attacked)
    with pytest.raises(HierarchicalStateV7ContractError, match="invalid-position"):
        load_frozen_parameter_bundle(
            attack_root,
...
def test_runtime_receipt_pins_outer32_inner1_cpu_gpu_and_py310() -> None:
    receipt = capture_runtime_receipt_v7(purpose="PREFLIGHT")
...
def test_source_audit_closes_v6_go_lineage_and_rejects_partial_registry_surfaces() -> None:
    audit = run_source_audit_v7(PROJECT_ROOT)
    assert audit.passed
    assert audit.v6_audit_raw_sha256 == V6_INDEPENDENT_AUDIT_BINDING["raw_sha256"]
    assert audit.v6_design_checksums_raw_sha256 == V6_DESIGN_FREEZE_BINDING["checksums_raw_sha256"]
...
def test_no_single_date_parameter_or_receipt_field_survives() -> None:
    names = {field.name for field in fields(FrozenHierarchicalParametersV7)}
...
def test_design_bundle_cross_seals_input_audit_and_contract(public_closure, tmp_path: Path) -> None:
    assert public_closure["geometry"]["fit_count"] == 3100
    bundle = freeze_design_v7.build_design_bundle_bytes(PROJECT_ROOT)
    root = tmp_path / "design"
    checksum = _write_payloads(root, bundle)
    receipt = freeze_design_v7.verify_design_bundle(
        root,
        expected_checksums_raw_sha256=checksum,
    )
    assert receipt["status"] == "PASS_EXACT_V7_DESIGN_BUNDLE_CLOSURE"
    assert set(bundle) == set(freeze_design_v7.FINAL_FILE_UNIVERSE)
    launcher_source = (
        PROJECT_ROOT
        / "scripts/model_lab/hierarchical_observable_fair_value_state_v7/prediction_launcher.py"
    ).read_bytes()
    assert bundle["PREDICTION_LAUNCHER.py"] == launcher_source
    launch_contract = json.loads(bundle["PREDICTION_LAUNCH_CONTRACT.json"].decode("ascii"))
    assert launch_contract["source_raw_sha256"] == hashlib.sha256(launcher_source).hexdigest()
    assert launch_contract["modes"] == ["check", "run"]
    unsigned_launch_contract = dict(launch_contract)
    unsigned_launch_contract.pop("manifest_sha256")
    assert unsigned_launch_contract == prediction_launcher_v7.launch_contract_payload(
        launcher_raw_sha256=hashlib.sha256(launcher_source).hexdigest(),
        design_contract_sha256=contract_sha256(),
    )
    assert launch_contract["execution_geometry"]["fit_count"] == 3100
    assert launch_contract["execution_geometry"]["decision_row_count"] == 64800
