...
from research.model_zoo.hierarchical_observable_fair_value_state_v5 import (
    ...
    FrozenHierarchicalParametersV5,
    HierarchicalStateV5ContractError,
    adapt_r4_canonical_source_v5,
    build_decision_batch_v5,
    build_frozen_parameter_bundle_bytes,
    build_hierarchical_state_features_v5,
    build_r4_fold_plan_v5,
    build_r4_input_closure_v5,
    capture_runtime_receipt_v5,
    contract_payload,
    contract_sha256,
    decision_block_membership_sha256_v5,
    fit_chronological_prefix_v5,
    load_frozen_parameter_bundle,
    run_frozen_decision_block_v5,
    run_source_audit_v5,
)
...
from scripts.model_lab.hierarchical_observable_fair_value_state_v5 import (
    freeze_design as freeze_design_v5,
)
...
def _block_identities(source: pd.DataFrame, start: int = 504) -> pd.DataFrame:
    state = build_hierarchical_state_features_v5(source)
...
def fitted_case():
    ...
    capability = fit_chronological_prefix_v5(
        source,
...
@pytest.fixture(scope="module")
def public_closure():
    return build_r4_input_closure_v5(PROJECT_ROOT)
...
def test_contract_lineage_geometry_runtime_and_new_identity() -> None:
    ...
    assert payload["candidate"]["candidate_id"] == (
        "hofs_v5_huber_soft_pool_state_dgp_r4_blocked"
    )
    assert payload["v4_independent_audit_binding"] == V4_INDEPENDENT_AUDIT_BINDING
    assert payload["v4_design_freeze_binding"] == V4_DESIGN_FREEZE_BINDING
    assert payload["r4_public_input_binding"] == R4_INPUT_BINDING
    assert V4_INDEPENDENT_AUDIT_BINDING["severity_counts"] == {"P0": 0, "P1": 0, "P2": 0}
...
def test_public_r4_closure_is_exact_and_score_blind(public_closure) -> None:
    ...
    assert public_closure["public_files_sha256"] == R4_INPUT_BINDING[
...
def test_public_canonical_adapter_is_schema_restricting() -> None:
    ...
    adapted = adapt_r4_canonical_source_v5(canonical)
...
def test_expected_regime_warmup_above_199_fails_closed() -> None:
    ...
    with pytest.raises(HierarchicalStateV5ContractError, match="sealed maximum"):
        build_hierarchical_state_features_v5(source)
...
def test_expected_regime_warmup_is_forbidden_in_decision_block() -> None:
    ...
    with pytest.raises(HierarchicalStateV5ContractError, match="forbidden in a decision block"):
        build_decision_batch_v5(state, requested)
...
def test_unexpected_decision_malformed_gate_uses_exact_block_denominator() -> None:
    ...
    state = build_hierarchical_state_features_v5(source)
    requested = _block_identities(source)
    with pytest.raises(HierarchicalStateV5ContractError, match="fallback limit"):
        build_decision_batch_v5(state, requested)
...
def test_prefix_below_504_and_interior_nonwarm_attacks_fail_before_solver() -> None:
    ...
    with pytest.raises(HierarchicalStateV5ContractError, match="prefix"):
        fit_chronological_prefix_v5(
            short,
            decision_block_identities=requested,
    ...
    with pytest.raises(HierarchicalStateV5ContractError, match="causal-first-row"):
        fit_chronological_prefix_v5(
            source,
            decision_block_identities=requested,
...
def test_valid_block_uses_one_parameter_hash_and_zero_updates(fitted_case) -> None:
    ...
    output = run_frozen_decision_block_v5(
        source,
    ...
    assert output.decision_block_membership_sha256 == decision_block_membership_sha256_v5(requested)
...
def test_removed_shifted_and_single_date_block_attacks_fail(fitted_case) -> None:
    source, requested, capability = fitted_case
    with pytest.raises(HierarchicalStateV5ContractError):
        run_frozen_decision_block_v5(
            source,
    ...
    with pytest.raises(HierarchicalStateV5ContractError, match="multiple"):
        build_decision_batch_v5(build_hierarchical_state_features_v5(source), single)


def test_update_and_membership_parameter_constructor_attacks_fail(fitted_case) -> None:
    _, _, capability = fitted_case
    parameters = capability.fit.parameters
    with pytest.raises(HierarchicalStateV5ContractError, match="block receipt"):
        replace(parameters, within_block_parameter_update_count=1)
    with pytest.raises(HierarchicalStateV5ContractError):
        replace(parameters, decision_block_membership_sha256="not-a-sha")
...
def test_parameter_artifact_round_trip_and_fully_resealed_membership_attack(
    ...
    with pytest.raises(HierarchicalStateV5ContractError, match="provenance drift"):
        load_frozen_parameter_bundle(
            attack_root,
...
def test_runtime_receipt_pins_outer32_inner1_cpu_gpu_and_py310() -> None:
    receipt = capture_runtime_receipt_v5(purpose="PREFLIGHT")
...
def test_source_audit_closes_v4_and_rejects_partial_registry_surfaces() -> None:
    audit = run_source_audit_v5(PROJECT_ROOT)
    assert audit.passed
    assert audit.v4_audit_raw_sha256 == V4_INDEPENDENT_AUDIT_BINDING["raw_sha256"]
    assert audit.v4_design_checksums_raw_sha256 == V4_DESIGN_FREEZE_BINDING[
...
def test_no_single_date_parameter_or_receipt_field_survives() -> None:
    names = {field.name for field in fields(FrozenHierarchicalParametersV5)}
    assert "decision_block_start_date" in names
    assert "decision_block_end_date" in names
    assert "decision_block_membership_sha256" in names
...
def test_design_bundle_cross_seals_input_audit_and_contract(public_closure, tmp_path: Path) -> None:
    assert public_closure["geometry"]["fit_count"] == 3100
    bundle = freeze_design_v5.build_design_bundle_bytes(PROJECT_ROOT)
    root = tmp_path / "design"
    checksum = _write_payloads(root, bundle)
    receipt = freeze_design_v5.verify_design_bundle(
        root,
        expected_checksums_raw_sha256=checksum,
    )
    assert receipt["status"] == "PASS_EXACT_V5_DESIGN_BUNDLE_CLOSURE"
    assert set(bundle) == set(freeze_design_v5.FINAL_FILE_UNIVERSE)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        "decision_block_membership_sha256",
        "decision_row_count",
