"""Score-free source/contract attacks for the isolated V2 R4 repair."""
...
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r4 import authority
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r4.artifacts import (
    canonical_json_bytes,
    seal_payload,
    sha256_file,
)
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r4.contracts import (
...
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r4.design import (
    CANDIDATE_CONTRACT,
...
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r4.executable import (
    executable_manifest_payload,
)
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r4.identity import (
    process_token_identity,
)
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r4.lineage import (
    R3_NO_GO_AUDIT_SEMANTIC,
    R3_NO_GO_SEAL_SEMANTIC,
    r3_supersession_payload,
)
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r4.path_guard import (
    R4PathError,
    require_clean_tree,
    require_lexically_exact_path,
)
from research.model_zoo.observable_state_bce_dgp_tournament_v2_r4.signature import (
    R4SignatureError,
...
PACKAGE = ROOT / "research/model_zoo/observable_state_bce_dgp_tournament_v2_r4"
BOOTSTRAP = ROOT / (
    "scripts/model_lab/observable_state_bce_dgp_tournament_v2_r4/"
    "trusted_bootstrap.py"
)
pytestmark = pytest.mark.skipif(sys.platform != "win32", reason="R4 is Windows-fixed")
...
def _valid_audit(context: dict[str, object]) -> dict[str, object]:
    return seal_payload(
        {
            "schema_version": authority.R4_AUDIT_SCHEMA,
            "audit_revision": "R4",
            "generated_at_utc": "2026-08-21T00:00:00+00:00",
            "status": authority.R4_AUDIT_STATUS,
            "verdict": authority.R4_AUDIT_VERDICT,
...
def _valid_approval(bindings: dict[str, object]) -> dict[str, object]:
    return seal_payload(
        {
            "schema_version": authority.R4_APPROVAL_SCHEMA,
            "status": authority.R4_APPROVAL_STATUS,
            "action": authority.R4_APPROVAL_ACTION,
            "decision": authority.R4_APPROVAL_DECISION,
...
def test_bootstrap_rejects_pyc_pyd_dll_and_extra_file(tmp_path: Path) -> None:
    ...
    for name in ("evil.py", "evil.pyc", "evil.pyd", "evil.dll", "evil.zip"):
        ...
        with pytest.raises(R4PathError):
            require_clean_tree(
...
def test_bootstrap_binds_all_initializers_origins_hashes_loaders() -> None:
    ...
    assert "research.model_zoo.observable_state_bce_dgp_tournament_v2_r4" in modules
    assert manifest["governed_loader_required"] == "__main__.BoundSourceLoader"
    assert set(path.name for path in PACKAGE.iterdir()) == set(R4_SOURCE_FILENAMES)
...
def test_bootstrap_verified_bytes_import_attestation_executes_before_action() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="r4_unit_source_only_") as temporary:
        stage = Path(temporary)
        trusted["_stage"](manifest, manifest_raw, stage)
        parent = os.environ
        environment = {
            "PYTHONDONTWRITEBYTECODE": "1",
            "R4_BOOTSTRAP_COMMAND": "attest_import_only",
            "R4_BOOTSTRAP_PARENT_NONCE": "a" * 64,
            "R4_EXECUTABLE_MANIFEST_RAW_SHA256": __import__("hashlib")
            .sha256(manifest_raw)
            .hexdigest(),
            "R4_STAGE_ROOT": str(stage),
...
def test_R3_NO_GO_full_bundle_supersession_exact() -> None:
    payload = r3_supersession_payload()
    assert payload["R3_NO_GO"]["audit_semantic_sha256"] == R3_NO_GO_AUDIT_SEMANTIC
    assert payload["R3_NO_GO"]["seal_semantic_sha256"] == R3_NO_GO_SEAL_SEMANTIC
    assert payload["R3_NO_GO"]["severity_counts"] == {"P0": 1, "P1": 1, "P2": 2}
...
def test_transaction_lock_is_process_lifetime_OS_lock() -> None:
    source = inspect.getsource(
        __import__(
            "research.model_zoo.observable_state_bce_dgp_tournament_v2_r4.transaction",
    ...
    with pytest.raises(R4TransactionError):
        _grant_bound_bootstrap_capability()
    with pytest.raises(R4TransactionError):
        _execute_authenticated_reservation(
            "RESERVE_OBSERVABLE_STATE_BCE_DGP_TOURNAMENT_V2_R4_TRANSACTIONALLY_ONCE",
            _BOUND_BOOTSTRAP_CAPABILITY,
        )
...
def test_named_test_results_and_raw_logs_bound() -> None:
    assert len(REQUIRED_NAMED_PROBES) == 21
    assert len(set(REQUIRED_NAMED_PROBES)) == 21
...
def test_audit_before_approval_and_freshness_enforced() -> None:
    source = inspect.getsource(authority.verify_root_reservation_approval_bundle)
...
def test_audit_minimal_unbound_extra_missing_wrong_hash_resealed_attacks_reject(
    ...
    with pytest.raises(authority.R4ContractError):
...
def test_minimal_unbound_extra_missing_wrong_hash_audit_approval_rejected(
    ...
    with pytest.raises(R4SignatureError):
